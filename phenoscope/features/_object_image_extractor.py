...
class ObjectImageExtractor(FeatureExtractor):
    ...
    def _operate(self, image: Image) -> Dict[int, Image]:
        ...
        obj_slices = pd.DataFrame(regionprops_table(image.obj_map, properties=['label', 'slice'])).set_index('label')
        ...
        for label in obj_slices.index:
            ...
            if self.target_dimensions is not None:
                ...
                if curr_obj_img.obj_mask is not None and curr_obj_img.obj_map is not None:
                    new_img.obj_mask = self._resize_object_mask(curr_obj_img)

                    obj_map = new_img.obj_mask
                    obj_map[new_img.obj] = label
                    new_img.obj_map = obj_map
    ...
    def _resize_object_mask(self, image):
        return resize(image=image.obj_mask, output_shape=self.target_dimensions,
                      anti_aliasing=False,
                      order=self.order,
                      anti_aliasing_sigma=self.anti_aliasing_sigma)
