...
class ObjectMask:
    ...
    def __setitem__(self, key, value: np.ndarray):
        ...
        self._handler.omap[:] = label(mask)

    @property
    def shape(self):
        return self._handler.omap.shape

    def copy(self) -> np.ndarray:
        """Returns a copy of the binary object mask"""
        return self._handler.omask[:].copy()

    def reset(self):
        self._handler.omap.reset()
    ...
    def show(self, ax: plt.Axes = None,
        ...
        ax.imshow(self._handler.omap[:] > 0, cmap=cmap)
    ...
    def _extract_objects(self, array: np.ndarray, bg_color: int = 0):
        ...
        if array.ndim == 3:
            mask = np.dstack(
                [(self._handler.omap[:] > 0) for _ in range(array.shape[-1])]
            )
        else: mask = self._handler.omap[:] > 0
        new_arr = array.copy()
