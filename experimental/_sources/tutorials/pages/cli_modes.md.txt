...
```{note}
**`recompile` writes no image store.** It reads each store's embedded
measurement table, projects it onto the store's own recorded
`measurement_columns`, concatenates the projections into
`master_measurements.parquet`, and joins `deliverables/metadata.csv` once at
finalization. No store's bytes, root, or per-image record are touched, so a
run built **with** `--metadata` recompiles exactly like one built without.

This was not always so. Until 2026-09-11 `recompile` re-joined the metadata
snapshot into every `tables/measurements/table.parquet` — the shape that
predates the split into a measurements table plus its own
`tables/metadata/pht-metadata.parquet` — and so had to refuse a `--metadata`
tree rather than un-invert it. The rewrite changed nothing the master or the
mirror could see once the join moved to finalization, so it was removed
rather than repointed, and the refusal went with it.
```
...
```
  ...
  A run that already had `--metadata` is fine too: the CSV you pass here
  replaces the snapshot the join reads, and no store is rewritten.
