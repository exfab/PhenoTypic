...
class GriddedImage(Image):
    def __init__(self, image: Union[np.ndarray, Image] = None, name: str = None, n_rows=8, n_cols=12,
        ...
        if gridding_method is None:
            self._grid_extractor = OptimalCenterGridSetter(nrows=n_rows, ncols=n_cols)
        else:
            self._grid_extractor = gridding_method
            self._grid_extractor.nrows = n_rows
            self._grid_extractor.ncols = n_cols
    ...
    @property
    def grid_extractor(self) -> GridSetter:
        return self._grid_extractor

    @grid_extractor.setter
    def grid_extractor(self, grid_extractor):
        if issubclass(grid_extractor, GridSetter):
            self._grid_extractor = grid_extractor
        else:
            raise ValueError('grid_setter should be an subclass of GridExtractor.')

    @property
    def n_rows(self):
        return self._grid_extractor.nrows

    @n_rows.setter
    def n_rows(self, value):
        self._grid_extractor.nrows = value

    @property
    def n_cols(self):
        return self._grid_extractor.ncols

    @n_cols.setter
    def n_cols(self, value):
        self._grid_extractor.ncols = value
