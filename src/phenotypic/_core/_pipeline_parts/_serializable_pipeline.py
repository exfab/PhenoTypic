...
class SerializablePipeline(ImagePipelineCore):
    ...
    def to_json_str(self):
        """
        Converts the internal configuration of the object into a JSON-formatted string.

        The generated JSON string provides a structured representation of the object's
        current state, including its operations, measurements, benchmarking state,
        and verbosity level. This output can be used for logging, debugging, or to
        recreate the object's configuration in another context.

        Returns:
            str: A JSON-formatted string that encodes the object's current configuration
            in a human-readable manner. This includes the lists of operations
    ...
    @classmethod
    def from_json_str(cls, json_str: str) -> ImagePipeline:
        """
        Deserialize a pipeline from a JSON string.

        This method reconstructs a pipeline from a JSON string generated by
        to_json_str(). It restores all operations, measurements, and configuration
        flags. Classes are imported from the phenotypic namespace and instantiated
        with their saved parameters.

        Args:
            json_str: A JSON-formatted string containing pipeline configuration.
        ...
        Example:
            .. dropdown:: Deserialize a pipeline from a JSON string

                >>> from phenotypic import ImagePipeline
                >>> pipe = ImagePipeline()
                >>> json_str = pipe.to_json_str()
                >>> restored_pipe = ImagePipeline.from_json_str(json_str)
        """
        # Parse JSON
        try:
