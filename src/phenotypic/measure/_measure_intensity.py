...
class MeasureIntensity(MeasureFeatures):
    ...
    def _operate(self, image: Image) -> pd.DataFrame:
        ...
        measurements = {
            str(INTENSITY.INTEGRATED_INTENSITY)          : self._calculate_sum(array=intensity_matrix, objmap=objmap),
            str(INTENSITY.MINIMUM_INTENSITY)             : self._calculate_minimum(array=intensity_matrix,
                                                                                   objmap=objmap),
            str(INTENSITY.MAXIMUM_INTENSITY)             : self._calculate_max(array=intensity_matrix, objmap=objmap),
            str(INTENSITY.MEAN_INTENSITY)                : self._calculate_mean(array=intensity_matrix, objmap=objmap),
            ...
            str(INTENSITY.Q1_INTENSITY)                  : self._calculate_q1(array=intensity_matrix, objmap=objmap),
            str(INTENSITY.Q3_INTENSITY)                  : self._calculate_q3(array=intensity_matrix, objmap=objmap),
            str(INTENSITY.IQR_INTENSITY)                 : self._calculate_iqr(array=intensity_matrix, objmap=objmap),
        }
