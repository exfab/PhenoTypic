...
class MeasureIntensity(MeasureFeatures):
    ...
    def _operate(self, image: Image) -> pd.DataFrame:
        ...
        measurements = {
            str(INTENSITY.INTEGRATED_INTENSITY): self.calculate_sum(array=intensity_matrix, labels=objmap),
            str(INTENSITY.MINIMUM_INTENSITY): self.calculate_minimum(array=intensity_matrix, labels=objmap),
            str(INTENSITY.MAXIMUM_INTENSITY): self.calculate_max(array=intensity_matrix, labels=objmap),
            str(INTENSITY.MEAN_INTENSITY): self.calculate_mean(array=intensity_matrix, labels=objmap),
            str(INTENSITY.MEDIAN_INTENSITY): self.calculate_median(array=intensity_matrix, labels=objmap),
            str(INTENSITY.STANDARD_DEVIATION_INTENSITY): self.calculate_stddev(array=intensity_matrix, labels=objmap),
            str(INTENSITY.COEFFICIENT_VARIANCE_INTENSITY): self.calculate_coeff_variation(
                array=intensity_matrix, labels=objmap,
            ),
            str(INTENSITY.Q1_INTENSITY): self.calculate_q1(array=intensity_matrix, labels=objmap),
            str(INTENSITY.Q3_INTENSITY): self.calculate_q3(array=intensity_matrix, labels=objmap),
            str(INTENSITY.IQR_INTENSITY): self.calculate_iqr(array=intensity_matrix, labels=objmap),
        }
