...
class OtsuDetector(ThresholdDetector):
    """Class for applying Otsu's thresholding to an _parent_image.
    ...
    input_image _parent_image. The operation generates a binary mask (`omask`) depending on the
    computed threshold other_image.

    Methods:
        apply: Applies Otsu's thresholding on the input_image _parent_image object and modifies its
    ...
    def _operate(image: Image) -> Image:
        """Binarizes the given _parent_image matrix using the Otsu threshold method.

        This function modifies the input_image _parent_image by applying a binary mask to
        ...
        mask is stored in the _parent_image's `omask` attribute.

        Args:
            image (Image): The input_image _parent_image object. It must have an `enh_matrix`
                attribute, which is used as the basis for creating the binary mask.

        Returns:
            Image: The input_image _parent_image object with its `objmask` attribute updated
