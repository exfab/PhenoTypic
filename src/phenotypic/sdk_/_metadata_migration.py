    ...
    *,
    ...
    if root is not None:
        ...
        try:
            ...
            raise ValueError(f"{role} escapes its authoritative root: {candidate}") from exc
    return candidate
...
def _known_header_map(columns: Iterable[object]) -> tuple[tuple[str, str], ...]:
...
def _normalize_mixed_table(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    normalized.columns = [
        restored_names.get(str(column), str(column)) for column in normalized.columns
    ]
    return normalized
...
def _target_status(
    ...
    *,
    ...
    if conflicts:
        return "blocked"
    return "migratable" if header_map or needs_metadata_marker else "compatible"
...
def _preflight_table(
    ...
    try:
        ...
        header_map = (
            _known_header_map(frame.columns) if mixed_table else _header_map(frame.columns)
        )
        if mixed_table:
            ...
            normalize_metadata_columns(frame)
        conflicts: tuple[str, ...] = ()
    except Exception as exc:  # malformed input is a blocked target, not a crash
        header_map = ()
...
def _normalize_json_column_reference(value: Any) -> tuple[Any, set[tuple[str, str]]]:
...
def _is_column_reference_field(class_name: str, class_: type[Any], field: str) -> bool:
    ...
    return model_field is not None and any(
        isinstance(metadata, _ColumnRefMarker) for metadata in model_field.metadata
    )
    ...
    if isinstance(params, dict):
        ...
        for field, raw_value in params.items():
            if _is_column_reference_field(class_name, class_, str(field)):
                normalized_value, child_maps = _normalize_json_column_reference(raw_value)
            else:
                normalized_value, child_maps = _normalize_nested_envelopes(raw_value)
...
def _known_inline_plot_class(module_name: Any, qualname: Any) -> type[Any] | None:
    """Resolve an inline plot class only from the PhenoTypic package."""
    if (
        not isinstance(module_name, str)
        or not isinstance(qualname, str)
        or not (module_name == "phenotypic" or module_name.startswith("phenotypic."))
...
def _normalize_inline_plot(
    ...
    mappings: set[tuple[str, str]] = set()
    for field, raw_value in params.items():
        if _is_column_reference_field(plot_class.__name__, plot_class, str(field)):
            normalized_value, child_maps = _normalize_json_column_reference(raw_value)
            normalized_params[field] = normalized_value
            mappings.update(child_maps)
        else:
...
def _normalize_nested_envelopes(value: Any) -> tuple[Any, set[tuple[str, str]]]:
...
def _normalize_pipeline_envelope(
    ...
    for key, value in payload.items():
        if key in {"pipe_cfgs", "meas", "post", "filters"} and isinstance(value, dict):
            normalized_slot: dict[Any, Any] = {}
            for name, entry in value.items():
                normalized_entry, child_maps = _normalize_nested_envelopes(entry)
...
def _inspect_hdf(
    ...
    with h5py.File(path, "r") as handle:
        ...
        for group in groups:
            ...
            for raw_header in group.attrs:
                ...
                aliases_by_target.setdefault(canonical, []).append(source_header)
    ...
    return header_map, tuple(conflicts), needs_metadata_marker, snapshot_fingerprint
...
def _discover_bundle_targets(layout: BundleLayout) -> tuple[Path, ...]:
    ...
    def validated_candidate(candidate: Path, root: Path, role: str) -> Path:
        if candidate.is_symlink():
            raise ValueError(f"Bundle-owned {role} cannot be a symlink: {candidate}")
    ...
    def validated_directory(candidate: Path, root: Path, role: str) -> Path:
        ...
        if not safe.is_dir():
            raise ValueError(f"Bundle-owned {role} is not a directory: {candidate}")
        return safe
    ...
    def legacy_root_pipeline() -> Path | None:
        ...
        if not state_path.is_file():
            return None
        safe_state = validated_candidate(state_path, bundle_root, "processing state")
        try:
        ...
        if not (candidate.exists() or candidate.is_symlink()):
            return None
        return validated_candidate(candidate, bundle_root, "legacy root pipeline")

    targets: list[Path] = []
    pipeline = layout.resolved_pipeline_config_path
    if pipeline.exists() or pipeline.is_symlink():
        targets.append(validated_candidate(pipeline, deliverables_root, "pipeline"))
    ...
    if layout.output_root is not None:
            ...
            for dataset_candidate in sorted(results_root.iterdir()):
                ...
                if hdf_candidate.exists() or hdf_candidate.is_symlink():
                    ...
                    for path in sorted(hdf_root.rglob("*")):
                        ...
                        if path.is_file() and path.suffix.lower() in _HDF_SUFFIXES:
                            targets.append(validated_candidate(path, hdf_root, "HDF"))
                ...
                individual = [
                    validated_candidate(path, validated_measurements, "measurement")
                    for path in sorted(validated_measurements.glob("*.parquet"))
                    if not path.name.startswith(("_", "."))
                ]
                targets.extend(individual)
                aggregate = validated_measurements / DATASET_AGGREGATED_PARQUET
                if not individual and (aggregate.exists() or aggregate.is_symlink()):
                    targets.append(
                        validated_candidate(
                            aggregate,
        ...
        for path in (layout.master_parquet, layout.master_csv):
            if path.exists() or path.is_symlink():
                targets.append(validated_candidate(path, deliverables_root, "master"))
...
def _report_from_targets(source: str, targets: tuple[MetadataMigrationTarget, ...]) -> MetadataMigrationReport:
    conflicts = tuple(conflict for target in targets for conflict in target.conflicts)
    if conflicts:
...
def preflight_metadata_schema(source: Any) -> MetadataMigrationReport:
    ...
    path = _require_safe_migration_path(source, role="Metadata preflight source")
    if path.is_file():
...
def _receipt_path(source: Path, plan_fingerprint: str, *, bundle: bool) -> Path:
    digest = plan_fingerprint.removeprefix("sha256:")[:16]
    return _receipt_dir(source, bundle=bundle) / f"metadata-schema-{digest}.json"
...
def _new_receipt(
    ...
    return {
        ...
        "targets": [
            {
                **asdict(target),
                "state": "skipped" if target.status == "compatible" else "pending",
...
def _validate_hdf_snapshot_semantics(
    ...
    *,
    ...
    if (
        ...
        or _hdf_snapshot_fingerprint(snapshot) != target.hdf_snapshot_fingerprint
    ):
    ...
    for raw_record in snapshot:
        ...
        if (
            ...
            raise ValueError("HDF affected attributes must be unique and sorted")
        represented = {
            source: canonical
            for source, canonical in planned
        ...
        if not represented:
            raise ValueError("HDF snapshot group contains no planned legacy header")
        expected_affected = sorted(
            {key for pair in represented.items() for key in pair}
        )
        if affected != expected_affected or not set(attributes).issubset(affected):
            raise ValueError("HDF snapshot attributes exceed the planned header map")
        for source, canonical in represented.items():
            ...
            if canonical in attributes and not _hdf_values_equal(
                ...
                raise ValueError("HDF snapshot contains conflicting convergent aliases")
    ...
    with h5py.File(path, "r") as handle:
        ...
        if set(marker_records) != expected_roots:
            raise ValueError("HDF marker snapshots do not match metadata roots")
        if not set(attr_records).issubset(metadata_groups):
            raise ValueError("HDF snapshot group is not a metadata attribute group")
            ...
            if phase == "original":
                for key in affected:
                    if (key in group.attrs) != (key in attributes):
                        raise ValueError("HDF original attribute state is incomplete")
                    if key in attributes and not _hdf_values_equal(
                        group.attrs[key], _decoded_snapshot_attr(attributes[key])
                    ):
                        raise ValueError("HDF original attribute snapshot does not match")
            else:
                for source in planned_by_source:
                    ...
                    if source in group.attrs or canonical not in group.attrs:
                        raise ValueError("HDF migrated attribute state does not match")
                    if not _hdf_values_equal(
                        group.attrs[canonical],
                        _decoded_snapshot_attr(attributes[source]),
                    ):
                        raise ValueError("HDF migrated attribute value does not match")
        ...
        for root_name, record in marker_records.items():
            ...
            if phase == "migrated":
                try:
                    ...
                    raise ValueError("HDF migrated metadata marker is invalid") from exc
                ...
                if (_METADATA_SCHEMA_ATTR in root.attrs) != marker_existed:
                    raise ValueError("HDF original metadata marker state is incomplete")
                if marker_existed and not _hdf_values_equal(
                    root.attrs[_METADATA_SCHEMA_ATTR],
                    _decoded_snapshot_attr(marker_value),
                ):
                    raise ValueError("HDF original metadata marker does not match")


def _hdf_inventory(path: Path, excluded_attrs: set[tuple[str, str]]) -> tuple[dict[str, str], dict[str, str]]:
    import h5py  # type: ignore[import-untyped]
...
def _migrate_hdf_copy(source: Path, temp: Path) -> list[dict[str, Any]]:
    ...
    if before_datasets != after_datasets or before_attrs != after_attrs:
        raise ValueError(f"HDF validation failed for {source}; non-target content changed")
    return snapshot


def _write_migrated_copy(target: Mapping[str, Any], temp: Path) -> dict[str, Any]:
    ...
    if kind == "json":
        ...
        temp.write_text(
            json.dumps(normalized, indent=2, sort_keys=False, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        shutil.copystat(source, temp)
        return {}
    ...
    if _hdf_snapshot_fingerprint(snapshot) != target.get(
        ...
        raise ValueError(f"HDF metadata state changed after preflight: {source}")
...
def _copy_backup(
    ...
    source = _require_safe_migration_path(source, role="Migration backup source")
    ...
    if backup.exists():
        if not backup.is_file() or backup.is_symlink():
            raise ValueError(f"Migration backup is not a regular file: {backup}")
        if file_fingerprint(backup) != source_fingerprint:
            raise ValueError(f"Existing migration backup has wrong fingerprint: {backup}")
        return backup
...
def _new_temp_path(source: Path) -> Path:
    ...
    handle = tempfile.NamedTemporaryFile(
        dir=source.parent, prefix=f".{source.name}.metadata-", suffix=".tmp", delete=False
    )
...
def _ensure_directory_durable(path: Path) -> None:
    ...
    while check != cursor:
        if check.is_symlink():
            raise ValueError(f"Migration directory cannot be a symlink: {check}")
        check = check.parent
...
def _receipt_target(record: Mapping[str, Any]) -> MetadataMigrationTarget:
    ...
    if status not in {"compatible", "migratable"}:
        raise ValueError(f"Invalid migration receipt target status: {status!r}")
    ...
    if kind == "hdf" and status == "migratable" and hdf_snapshot_fingerprint is None:
    ...
    return MetadataMigrationTarget(
        ...
        source_fingerprint=fingerprint,
        proposed_header_map=tuple((str(pair[0]), str(pair[1])) for pair in raw_map),
...
def _validate_receipt(
    ...
    *,
    ...
    if receipt.get("state") not in {"prepared", "applied", "failed", "rolled_back"}:
    ...
    if not isinstance(source_fingerprint, str) or not source_fingerprint.startswith(
        "sha256:"
    ):
    ...
    if expected_plan_fingerprint is not None and plan_fingerprint != expected_plan_fingerprint:
        raise ValueError("Migration receipt plan fingerprint does not match request")
    ...
    if scope == "file":
        ...
        if receipt.get("bundle_root") is not None:
            raise ValueError("File migration receipt cannot declare a bundle root")
        expected_paths = (source,)
        receipt_source = str(source)
        expected_receipt = _receipt_path(source, plan_fingerprint, bundle=False)
    else:
        raw_root = receipt.get("bundle_root")
        if not isinstance(raw_root, str):
            raise ValueError("Bundle migration receipt is missing its root")
        root = _require_safe_migration_path(raw_root, role="Migration bundle root")
        ...
        if source_text == str(full_deliverables):
            layout = BundleLayout(deliverables_base=full_deliverables, output_root=root)
        elif source_text == str(root):
            layout = BundleLayout(deliverables_base=root, output_root=None)
        else:
            raise ValueError("Bundle receipt source is not rooted in its bundle")
    ...
    if source_text != receipt_source:
        raise ValueError("Migration receipt source does not match its resolved scope")
    ...
    if receipt_path != expected_receipt:
        raise ValueError("Migration receipt is outside its authoritative journal")
    ...
    for target, expected_path in zip(targets, expected_paths, strict=True):
        ...
        if target.kind != _kind_for_file(expected_path):
            raise ValueError("Migration receipt target kind does not match its path")
    ...
    if rebuilt.source_fingerprint != source_fingerprint:
        raise ValueError("Migration receipt source fingerprint has been altered")
    ...
    for record, target, target_path in zip(
        ...
        if state not in {"pending", "prepared", "applied", "skipped", "rolled_back"}:
            raise ValueError("Invalid migration receipt target state")
        if state == "skipped" and record.get("status") != "compatible":
            raise ValueError("Only compatible migration targets may be skipped")
        if state != "skipped" and record.get("status") != "migratable":
            raise ValueError("Compatible migration targets must remain skipped")
        ...
        if state in {"prepared", "applied"} and post_fp is None:
            raise ValueError("Prepared migration target lacks a post fingerprint")
        ...
        if target.kind != "hdf" and rollback_fp is not None:
            raise ValueError("Non-HDF target cannot carry a rollback fingerprint")
        if target.kind == "hdf":
            if state == "rolled_back" and rollback_fp is None:
                raise ValueError("Rolled-back HDF target lacks its fingerprint")
            if rollback_fp is not None and state not in {
                "pending",
                "prepared",
                "rolled_back",
            }:
                raise ValueError("HDF rollback fingerprint has an invalid lifecycle state")
        ...
        if backup_text is not None:
            ...
            if backup.parent != backups_dir:
                raise ValueError("Migration backup is outside the receipt backup directory")
            if record.get("kind") == "hdf":
                raise ValueError("HDF migration receipts cannot contain file backups")
            if not backup.is_file() or file_fingerprint(backup) != source_fp:
                raise ValueError("Migration backup is missing or has the wrong fingerprint")
        elif state in {"prepared", "applied", "rolled_back"} and record.get("kind") != "hdf":
        ...
        if temp_text is not None:
            ...
            if temp.exists() and (
                not temp.is_file() or post_fp is None or file_fingerprint(temp) != post_fp
            ):
                raise ValueError("Prepared migration temp has the wrong fingerprint")
        ...
        if current_fp not in allowed_current:
            raise ValueError(f"Migration receipt target fingerprint changed: {target_path}")
        if target.kind == "hdf":
            snapshot = record.get("hdf_snapshot")
            if state == "skipped" or (state == "pending" and rollback_fp is None):
                if snapshot is not None:
                    raise ValueError("Unprepared HDF migration target has a snapshot")
            else:
                phase: Literal["original", "migrated"] = (
                    "original"
            ...
            raise ValueError("Non-HDF migration target contains an HDF snapshot")
    ...
    *,
    ...
    try:
        for target in receipt["targets"]:
            ...
            if state == "skipped":
                if file_fingerprint(path) != target["source_fingerprint"]:
                    raise ValueError(f"Skipped migration target changed: {path}")
                skipped.append(str(path))
                continue
            if state == "applied":
                if file_fingerprint(path) != target["post_fingerprint"]:
                    raise ValueError(f"Applied migration target changed: {path}")
            ...
            if state == "rolled_back":
                ...
                if file_fingerprint(path) != expected_rollback:
                    raise ValueError(f"Rolled-back migration target changed: {path}")
            ...
            if state == "prepared":
                ...
                if not temp.is_file() or file_fingerprint(temp) != target["post_fingerprint"]:
                    target["state"] = "pending"
            ...
            if isinstance(target.get("rollback_fingerprint"), str):
                accepted_prepublication.add(str(target["rollback_fingerprint"]))
            if file_fingerprint(path) not in accepted_prepublication:
                raise ValueError(f"Migration target changed before publication: {path}")
            _publish_temp(temp, path)
            if file_fingerprint(path) != target["post_fingerprint"]:
                raise ValueError(f"Published migration target failed validation: {path}")
            target["state"] = "applied"
            target["temp_path"] = None
            target["rollback_fingerprint"] = None
            _write_receipt(receipt_path, receipt)
        ...
        result_fingerprint = _sha256_bytes(
            json.dumps(
                [(target["path"], target.get("post_fingerprint") or target["source_fingerprint"]) for target in receipt["targets"]],
                sort_keys=True,
                separators=(",", ":"),
            ).encode()
        )
...
def _blocked_result(report: MetadataMigrationReport) -> MetadataMigrationResult:
    return MetadataMigrationResult(
        ...
        blocked_targets=tuple(
            target.path for target in report.targets if target.status == "blocked"
        ),
        conflicts=report.conflicts,
    )


def _compatible_result(report: MetadataMigrationReport) -> MetadataMigrationResult:
    return MetadataMigrationResult(
        status="compatible",
        source=report.source,
        source_fingerprint=report.source_fingerprint,
...
def migrate_metadata_file(
    ...
    if expected_source_fingerprint not in {target_fingerprint, report.source_fingerprint}:
...
def _resolve_bundle(source: str | Path | BundleLayout) -> tuple[BundleLayout, Path]:
    if isinstance(source, BundleLayout):
        layout = source
        root = layout.output_root or layout.deliverables_base
        return layout, _require_safe_migration_path(root, role="Migration bundle root")
    requested = _require_safe_migration_path(source, role="Migration bundle source")
...
def migrate_metadata_bundle(
    ...
    requested_receipt = _receipt_path(root, expected_plan_fingerprint, bundle=True)
    _require_safe_migration_path(
        requested_receipt,
...
def rollback_metadata_migration(receipt_path: str | Path) -> MetadataMigrationResult:
    ...
    try:
        for target in reversed(receipt["targets"]):
            ...
            if target["state"] == "prepared" and current == prepared_origin_fingerprint:
                ...
                continue
            if current != target["post_fingerprint"]:
                raise ValueError(f"Cannot rollback changed migration target: {source}")
            if target["kind"] == "hdf":
                _rollback_hdf(
                    source,
                ...
                if not backup.is_file() or file_fingerprint(backup) != target["source_fingerprint"]:
                    raise ValueError(f"Migration backup is missing or changed: {backup}")
                temp = _new_temp_path(source)
                try:
        ...
        return MetadataMigrationResult(
            ...
            resulting_fingerprint=_sha256_bytes(
                json.dumps(
                    [
                        (target["path"], file_fingerprint(Path(target["path"])))
                        for target in receipt["targets"]
                    ],
                    sort_keys=True,
                    separators=(",", ":"),
