...
#: ``ProcessingState.load`` during automatic continuation.
...
#: Marker-last evidence for one internally consistent aggregate snapshot.
AGGREGATE_PUBLICATION_JSON: Final[str] = "aggregate_publication.json"
...
#: Authoritative append-only journal of terminal per-image scientific failures.
#: Unlike ``FAILURES_JSONL``, this file lives directly under ``.phenotypic/``
#: because progress artifacts are rebuildable display state.
TERMINAL_FAILURES_JSONL: Final[str] = "terminal_failures.jsonl"
...
def terminal_failures_jsonl_path(output_dir: Path) -> Path:
    """Return ``<output>/.phenotypic/terminal_failures.jsonl``."""
    return phenotypic_cache_dir(output_dir) / TERMINAL_FAILURES_JSONL
...
def aggregate_publication_marker_path(output_dir: Path) -> Path:
    """Return ``<output>/.phenotypic/aggregate_publication.json``."""
    return phenotypic_cache_dir(output_dir) / AGGREGATE_PUBLICATION_JSON


def image_completion_marker_path(
