...
toggle, inline lifecycle checkboxes, an Advanced collapse
(``--sample``, ``--nrows``, ``--ncols``,
...
Three :class:`dbc.Modal` factories are also defined here:
    ...
    * :func:`build_pipeline_picker_modal` — wraps
      :func:`~phenotypic._gui.builder._directory_browser.directory_tree`
...
from phenotypic._gui._config import IMAGE_EXTS
from phenotypic._gui._design import FONT_FAMILY_MONO
...
def build_form(sandbox: SandboxRoot) -> html.Div:
    ...
    Sections (top to bottom):
        ...
        3. Inline checkboxes (``Dry-run``, ``Retry terminal failures``).
        ...
        [
            dbc.Checklist(
                ...
                options=[
                    ...
                    {"label": "Retry terminal failures", "value": "retry_failures"},
                ],
                value=[],
                inline=True,
            ),
