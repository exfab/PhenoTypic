...
chosen directory is validated by :meth:`~phenotypic._gui.tune.TuneRunRoot.discover`
and, on success, its path is written into
:data:`~phenotypic._gui.tune._ids.TUNE_RUN_ROOT_STORE` — the store the Monitor /
...
This mirrors the Curate :mod:`~phenotypic._gui.tune._image_source` picker exactly
...
from phenotypic._gui.tune._run_root import TuneRunRoot, TuneRunRootError
...
def discover_run_payload(
    ...
    :meth:`~phenotypic._gui.tune.TuneRunRoot.discover`, and on success returns the
    JSON-serialisable run-root payload the
    :data:`~phenotypic._gui.tune._ids.TUNE_RUN_ROOT_STORE` carries (a ``{"path":
