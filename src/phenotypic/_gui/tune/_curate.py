...
* the **shortlist** (B4) — one clickable card per
  :func:`~phenotypic._gui.tune._study_read.shortlist` trial, pinned to slot A or B;
...
:class:`~phenotypic._gui.tune._overlays.OverlayCache` singleton); the Curate
...
from phenotypic._gui.tune._figures import transparent_layout
...
def _shortlist_cards(root: "TuneRunRoot") -> list[Component]:
    ...
    """
    from phenotypic._gui.tune._callbacks import _load_journal  # optuna-free
    from phenotypic._gui.tune._study_read import shortlist
...
def build_curate_view(
    ...
    if sandbox is not None:
        from phenotypic._gui.tune._image_source import build_image_source_modal
