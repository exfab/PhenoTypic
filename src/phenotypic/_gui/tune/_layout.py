...
and a swappable :data:`~phenotypic._gui.tune._ids.TUNE_PAGE_BODY` container — that
...
:mod:`phenotypic._gui._design` (via class names that reference the injected
...
from phenotypic._gui.tune._run_picker import build_run_picker_modal, build_run_picker_row
...
from phenotypic._gui.tune._command import DEFAULT_STORAGE_ENV
...
def _build_view_body(
    ...
    if name == "curate":
        from phenotypic._gui.tune._curate import build_curate_view

        return build_curate_view(root, sandbox=sandbox)
    if name == "launch":
        from phenotypic._gui.tune._launch import build_launch_view

        return build_launch_view(root)
    if name == "space":
        from phenotypic._gui.tune._space import build_space_view
