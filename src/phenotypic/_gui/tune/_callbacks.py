...
* **Run binding** — :func:`~phenotypic._gui.tune._run_picker.discover_run_payload`
...
from phenotypic._gui.tune import _nav
from phenotypic._gui.tune._command import (
    DEFAULT_STORAGE_ENV,
    StorageMode,
    build_tune_command,
    storage_url_preflight_issue,
)
from phenotypic._gui.tune._deploy import deploy_tune_run
...
from phenotypic._gui.tune._validation import preflight_issues, spec_path_issue
...
if TYPE_CHECKING:
    ...
    from phenotypic._gui.tune._study_read import _ReadableStore
...
def _gap_badge_outputs(store: "Optional[_ReadableStore]") -> tuple[str, str]:
    ...
    from phenotypic._gui.tune._study_read import gap_badge
...
def _register_setup_picker_callbacks(app, sandbox) -> None:  # type: ignore[no-untyped-def]
    ...
    from phenotypic._gui.builder._directory_browser import directory_tree
...
def register_callbacks(app, *, sandbox=None) -> None:  # type: ignore[no-untyped-def]
    ...
    * **Curate** (when ``sandbox`` is bound) — the sandbox-bounded Image Source
    ...
    def _render_setup_draft(
        ...
        try:
            from phenotypic._gui.tune._space import setup_knob_forms
...
def _register_run_picker_callbacks(app, sandbox) -> None:  # type: ignore[no-untyped-def]
    ...
    * **Confirm → bind**: validate the chosen directory via
      :func:`~phenotypic._gui.tune._run_picker.discover_run_payload`, and on
    ...
    from phenotypic._gui.tune._layout import build_loaded_body
    from phenotypic._gui.tune._run_picker import (
...
def _register_launch_command_mirror(app) -> None:  # type: ignore[no-untyped-def]
    ...
    :func:`~phenotypic._gui.tune._command.render_launch_command` (the unit-tested
...
def _collect_space_edits(
    ...
    *,
    ...
    yields an empty dict, which :func:`~phenotypic._gui.tune._space._apply_edits`
...
def write_space_spec(
    ...
    :func:`~phenotypic._gui.tune._space.space_to_spec` (preserving the run's
    ...
    """
    from phenotypic._gui.tune._space import _load_space_source, space_to_spec
...
def _register_curate_callbacks(app, sandbox) -> None:  # type: ignore[no-untyped-def]
    ...
    from phenotypic._gui.tune._image_source import (
...
def _write_curate_winner(ov, n_clicks, pinned, run_root_data):  # type: ignore[no-untyped-def]
    ...
    :func:`~phenotypic._gui.tune._winner.write_winner`, and catches
...
def _submit_curate_overlays(  # type: ignore[no-untyped-def]
    ...
    *,
    ...
    from phenotypic._gui.tune import _curate as curate
...
def _submit_difference(  # type: ignore[no-untyped-def]
    ...
    def _render():  # type: ignore[no-untyped-def]
        from phenotypic._gui.tune._overlays import OVERLAY_MAX_DIM, render_difference
