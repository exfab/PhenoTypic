...
Mirrors :mod:`phenotypic._gui.results_viewer._app` and
:mod:`phenotypic._gui.analysis._app`: builds a configured :class:`dash.Dash`
whose layout is the tune page for a bound :class:`~phenotypic._gui.tune.TuneRunRoot`,
...
:mod:`phenotypic._gui.tune` — must not pull ``optuna`` into ``sys.modules``.
"""
...
from phenotypic._gui._design import inject_design_tokens
from phenotypic._gui._shared import register_shared_static
from phenotypic._gui._url_prefix import configure_url_prefix_routing
from phenotypic._gui.tune._callbacks import register_callbacks
from phenotypic._gui.tune._layout import build_layout
    ...
    *,
    ...
    Args:
        root: Validated tune output handle (see
            :meth:`phenotypic._gui.tune.TuneRunRoot.discover`). ``None``
        ...
        url_prefix: Mount-point prefix passed to ``dash.Dash`` as
            ...
            :func:`phenotypic._gui.shell._app.compose_hub` passes ``MOUNT_TUNE``.
    ...
    Notes:
        ...
        :func:`~phenotypic._gui.tune._callbacks.register_callbacks`), even for the
