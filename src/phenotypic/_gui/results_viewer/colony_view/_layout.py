...
DESIGN.md tokens are honoured throughout via ``phenotypic._gui._design``:
...
from phenotypic._gui._config import COLONY_TILE_SIZE_DEFAULT, TILE_DIM_DEFAULT
...
from phenotypic._gui._shared.tiles import DEFAULT_LAYER, LayerName
...
#: :data:`phenotypic._gui._shared.tiles.LayerName` members; the human labels map
...
def build_layer_toggle(output_root: OutputRoot) -> Component | None:
    ...
    Args:
        output_root: Validated handle on the CLI output directory. Only its
            :attr:`~phenotypic._gui.results_viewer._output_root.OutputRoot.has_results`
            flag is read.
...
def _build_dim_stepper(
    *,
    ...
    :data:`phenotypic._gui.results_viewer._ids.STORE_TILE_DIM_ALPHA`
