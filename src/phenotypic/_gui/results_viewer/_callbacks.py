...
2. **Clientside bridge: source spec -> Viv mount.** Each card's
   :data:`~phenotypic._gui.results_viewer._ids.card_source_store_id`
   ``dcc.Store`` holds ``build_source_spec``'s dict for the selected
   image's OME-Zarr store, and its
   :data:`~phenotypic._gui.results_viewer._ids.card_display_state_id`
...
3. **Clientside bridge: lock-views toggle -> viewport broadcast.** A
   ...
   :data:`~phenotypic._gui.results_viewer._ids.STORE_LOCK_VIEWS` and
   forwards changes to
...
instances (:data:`~phenotypic._gui.results_viewer._ids
.VIV_MOUNT_TRIGGER_ID`,
:data:`~phenotypic._gui.results_viewer._ids.LOCK_VIEWS_EFFECT_ID`)
...
from phenotypic._gui.results_viewer._filtered_state import get_curated_frame
...
from phenotypic._gui.results_viewer._scatter_tab import (
...
def _register_plot_refresh_callback(
    ...
    def _refresh_measurement_plots(
        ...
        try:
            ...
            from phenotypic._gui._plot_refresh import refresh_measurement_plots
