...
* :func:`register_point_picker_routes` — Flask blueprint mounted at
  ...
  pyramids using :mod:`phenotypic._gui.results_viewer._dzi_tiler`.
...
:mod:`phenotypic._gui._shared.tiles`, so this blueprint keeps the same
...
def _resolve_node_and_predecessor(
    ...
    if hasattr(state, "selected_block_id"):
        ...
        if not isinstance(root_dict, dict):
            return (None, None)
        from phenotypic._gui.builder._callbacks import _find_block_in_tree
