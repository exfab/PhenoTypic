...
class ObjectMask(ImageArrDataAccessor):
    """Represents a binary object mask linked to a parent _root_image.

    This class allows for manipulation and analysis of a binary object mask associated with a parent _root_image. It provides
    functionality to access, modify, display, and extract object regions of the mask. The object mask is tightly linked
    to the parent _root_image, which is used as the source for the binary map.
    ...
    def shape(self):
        """
        Represents the shape of a parent _root_image's omap property.

        This property is a getter for retrieving the shape of the `omap` attribute
        of the associated parent _root_image.
    ...
    def reset(self):
        """
        Resets the overlay map (omap) tied to the parent _root_image. This function interacts with
        the `omap` object contained within the parent _root_image, delegating the reset operation
        to it.
    ...
    def show(self, ax: plt.Axes | None = None,
        ...
        Calls object_map linked by the _root_image handler
