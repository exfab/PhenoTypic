...
class GridFinder(GridMeasureFeatures):
    ...
    def _add_row_interval_info(self, table: pd.DataFrame, row_edges: np.array, imshape: (int, int)) -> pd.DataFrame:
        row_edges = self._clip_row_edges(row_edges=row_edges, imshape=imshape)
        table.loc[:, GRID_DEP.GRID_ROW_INTERVAL] = pd.cut(
            table.loc[:, str(BBOX.CENTER_RR)],
            bins=row_edges,
    ...
    def _add_col_number_info(self, table: pd.DataFrame, col_edges: np.array, imshape: (int, int)) -> pd.DataFrame:
        ...
        table.loc[:, GRID_DEP.GRID_COL_NUM] = pd.cut(
    ...
    def _add_col_interval_info(self, table: pd.DataFrame, col_edges: np.array, imshape: (int, int)) -> pd.DataFrame:
        col_edges = self._clip_col_edges(col_edges=col_edges, imshape=imshape)
        table.loc[:, GRID_DEP.GRID_COL_INTERVAL] = pd.cut(
            table.loc[:, str(BBOX.CENTER_CC)],
            bins=col_edges,
    ...
    def _add_section_interval_info(self, table: pd.DataFrame,
        ...
        if str(GRID.ROW_NUM) not in table.columns: self._add_row_number_info(table=table, row_edges=row_edges, imshape=imshape)
        if GRID_DEP.GRID_COL_NUM not in table.columns: self._add_col_number_info(table=table, col_edges=col_edges, imshape=imshape)
        table.loc[:, GRID_DEP.GRID_SECTION_IDX] = list(zip(table.loc[:, str(GRID.ROW_NUM)], table.loc[:, GRID_DEP.GRID_COL_NUM]))
        table.loc[:, GRID_DEP.GRID_SECTION_IDX] = table.loc[:, GRID_DEP.GRID_SECTION_IDX].astype('category')
        return table

    def _add_section_number_info(self, table: pd.DataFrame,
                                 row_edges: np.array, col_edges: np.array,
                                 imshape: (int, int)) -> pd.DataFrame:
        if GRID_DEP.GRID_SECTION_IDX not in table.columns: self._add_section_interval_info(
        ...
        for idx in np.sort(np.unique(table.loc[:, GRID_DEP.GRID_SECTION_IDX].values)):
            table.loc[table.loc[:, GRID_DEP.GRID_SECTION_IDX] == idx, GRID_DEP.GRID_SECTION_NUM] = idx_map[idx[0], idx[1]]

        table.loc[:, GRID_DEP.GRID_SECTION_NUM] = table.loc[:, GRID_DEP.GRID_SECTION_NUM].astype('category')
        return table
