...
class GridFinder(GridMeasureFeatures):
    ...
    def _add_section_interval_info(self, table: pd.DataFrame,
        ...
        if str(GRID.COL_NUM) not in table.columns: self._add_col_number_info(table=table, col_edges=col_edges, imshape=imshape)
        table.loc[:, str(GRID.SECTION_IDX)] = list(zip(table.loc[:, str(GRID.ROW_NUM)], table.loc[:, str(GRID.COL_NUM)]))
