...
shared base layout (body font + transparent paper/plot backgrounds) so the
three call sites can't drift on the font token or the transparent sentinel.
...
def transparent_layout(**overrides: object) -> dict[str, object]:
    """The shared tune-figure layout: body font + transparent paper/plot fill.

    Returns a fresh ``dict`` (safe to splat into ``update_layout``) carrying the
    design-token body font and the transparent paper/plot backgrounds every tune
