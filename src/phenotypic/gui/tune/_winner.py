...
def write_winner(
    ...
    # Atomic write: temp file in the SAME directory (so os.replace is a rename,
    # not a cross-device copy), then os.replace onto the target.
    fd, tmp_name = tempfile.mkstemp(
        dir=str(target.parent), prefix=".best_pipeline.", suffix=".json.tmp"
    )
    try:
        ...
        # Clean up the temp file on any failure (including the PermissionError
        # we re-raise) so a read-only dir doesn't accrete .tmp turds.
        try:
            os.unlink(tmp_name)
        except OSError:
            pass
