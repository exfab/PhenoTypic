...
| run_local      | Run locally          | Run console form, pipeline / input / output pickers, generation receipts, callback-error acknowledgement, terminal Cancel gating, and retained logs. | `_capture_run_local`      | `gui/04_run_local.md`        | ✅ shipping |
...
| tune_copilot | Tune co-pilot | Drive the `/tune/` co-pilot through **Setup** (path-backed, fingerprinted authoring that preserves an existing spec), **Run** (source/output/strategy/budget/storage/compute/evaluation with a command card identical to Deploy argv), and **Monitor**. On Monitor read trial progress, switch registered runs, cancel Local-only runs, and export the best pipeline; use Curate, Space, and Launch for overlay comparison, search-space review, and a next-run command. Existing outputs bind read-only through the sandbox-bounded picker. | `_capture_tune_copilot` | `gui/16_tune_copilot.md` | ✅ shipping |
