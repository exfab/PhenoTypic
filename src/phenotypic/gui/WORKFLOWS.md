...
| tune_copilot | Tune co-pilot | Open the `/tune/` co-pilot in its empty state, then **bind a tune output directory** with the sandbox-bounded run picker (Browse → pick the run dir → Bind this run) so the page swaps in the loaded views. Read live trial progress on **Monitor** (objective + importance figures, gap badge, trials table), switch among registered Local/SLURM runs, stop Local-only runs, and export the best pipeline from `best_params.json`; pin two trials A/B and compare their colony overlays on **Curate**, review the inferred search space on **Space**, and render the next `run` invocation on **Launch**. | `_capture_tune_copilot` | `gui/16_tune_copilot.md` | ✅ shipping |
