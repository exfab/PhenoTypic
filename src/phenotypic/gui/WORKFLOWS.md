...
| run_local      | Run locally          | Run console form, pipeline / input / output pickers, direct progress-only dashboard, generation receipts, uncertain acknowledgement handling, terminal Cancel gating, and retained logs. | `_capture_run_local`      | `gui/04_run_local.md`        | ✅ shipping |
| run_slurm      | Run on SLURM         | SLURM mode toggle, advanced config, Progress/Download dashboard tabs, `sbatch --export=ALL`, and durable generation-bound ordinary/staged lifecycle observation, logs, cancellation, publication, restart guidance. | `_capture_run_slurm`      | `gui/05_run_slurm.md`        | ✅ shipping |
...
| qc_curation_loop | QC curation loop | Configure Count + SE checks, use explicit compatibility migration/rebuild when required, then watch metrics improve as you curate flagged colonies | `_capture_qc_curation_loop` | `gui/10_qc_curation_loop.md` | ✅ shipping |
...
| qc_review | QC review walkthrough | Switch the QC tab to Review; pick a module, walk its worst-first worklist, curate flagged colonies in the faceted tile gallery, mark a group reviewed, and watch the metric recompute in place (before→after delta). Per-module review progress persists in `deliverables/qc/review_state.json` and resets on the next CLI recompile. | `_capture_qc_review` | `gui/15_qc_review.md` | ✅ shipping |
| error_analysis | Error analysis | Switch the Results Viewer to **Error**, preview category cutoffs and good-baseline choices without writing, then explicitly **Publish all categories** as one complete transactional generation with a receipt. | `_capture_error_analysis` | `gui/17_error_analysis.md` | ✅ shipping |
...
| tune_copilot | Tune co-pilot | Drive the `/tune/` co-pilot through **Setup** (path-backed, fingerprinted authoring that preserves an existing spec), **Run** (source/output/strategy/budget/storage/compute/evaluation with a command card identical to Deploy argv), and **Monitor**. Monitor selection, logs, reconciliation, cancel, and export retain the exact run-id + generation receipt; use Curate, Space, and Launch for overlay comparison, search-space review, and a next-run command. Existing outputs bind read-only through the sandbox-bounded picker. | `_capture_tune_copilot` | `gui/16_tune_copilot.md` | ✅ shipping |
