...
| postMessage (dashboard → parent) | JS in `_cli/_dashboard/_generator.py` | Guarded by `window.parent !== window`; standalone path silent              | 🔭 planned | integration | tests/integration/gui/test_postmessage_listener.py::test_listener_receives |

## Cross-cutting infrastructure

| Feature                       | Element                                    | Expected behaviour                                                                 | Status     | Test layer  | Test ref                                                                      |
| ----------------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------- | ---------- | ----------- | ----------------------------------------------------------------------------- |
| SandboxRoot containment       | `resolve()`                                | Rejects out-of-root and out-of-root symlinks with `ValueError`                     | 🔭 planned | unit        | tests/unit/gui/shell/test_sandbox.py::test_resolve_rejects_outside_root       |
| Capability classifier         | `classify(path)`                           | Stat-only; LRU-cached on `(path, mtime)`                                           | 🔭 planned | unit        | tests/unit/gui/shell/test_classifier.py::test_classify_images_only            |
| ToolSession lifecycle         | `get`/`touch`/`idle_seconds`/`release`     | Lazy build; idempotent release; thread-safe                                        | 🔭 planned | unit        | tests/unit/gui/shell/test_session.py::test_lazy_build                         |
| RunRegistry concurrency       | Locked register/get/list/update_status     | Concurrent updates serialize via `threading.Lock`                                  | 🔭 planned | unit        | tests/unit/gui/shell/test_runs_registry.py::test_concurrent_updates           |
| LocalRunner ring buffer       | `collections.deque(maxlen=5000)`           | No subprocess pipe deadlock under flood                                            | 🔭 planned | unit        | tests/unit/gui/run_console/test_runner.py::test_no_deadlock_under_flood       |
| `/sandbox/api/*` blueprint    | Flask blueprint on `shell_app.server`      | Returns JSON for root/children/classify; respects sandbox                          | 🔭 planned | integration | tests/integration/gui/test_routes.py::test_sandbox_api_shapes                 |
| `/runs/<rel>/<path:file>` BP  | Flask blueprint on `shell_app.server`      | Serves files under sandbox; rejects path traversal; touches viewer session         | 🔭 planned | integration | tests/integration/gui/test_runs_blueprint.py::test_dashboard_html_served      |
