...
| Filter section stack          | `#analysis-filter-stack` + `_choices_for_category("Filter")` | Authors `pipeline.filters`; choices auto-discovered from `OperationRegistry`; same add/remove pattern; plot preview deferred to v2.                          | ✅ shipping | e2e         | tests/e2e/gui/test_analysis_app.py::test_analysis_standalone_renders_pipeline_header      |
