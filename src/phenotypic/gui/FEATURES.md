...
The aux node hangs off a dock area below the main ribbon; a purple dashed
edge wires it into the consumer's port handle. Inspector rows reflect
wired vs. empty state, and list-typed ports support ``+`` / ``×`` slot
controls. Save drops orphan aux nodes with a toast warning; drilling
into an aux node pushes a breadcrumb segment for editing aux-of-aux.

| Feature                          | Element                                              | Expected behaviour                                                                                                                                  | Status     | Test layer  | Test ref                                                                                                                  |
| -------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------- |
| Aux port handle on canvas        | Cytoscape sub-node on consumer's left edge           | Each aux input parameter renders as a port handle on the consumer step's left edge; serialized state round-trips through `BuilderScope.aux_nodes`.   | ✅ shipping | integration | tests/gui/builder/test_state_roundtrip.py::test_filamentous_fungi_detector_extracts_to_aux_node                           |
| Aux node dock                    | Dock area below the main ribbon                      | Aux operation/pipeline nodes hang off a dock area below the main ribbon; multiple aux nodes coexist for a single consumer with multiple aux inputs.  | ✅ shipping | integration | tests/gui/builder/test_state_roundtrip.py::test_composite_detector_extracts_two_aux_nodes                                 |
| Aux wire (purple dashed)         | Cytoscape edge between aux source and consumer port  | Wires render as a purple dashed cytoscape edge to distinguish from the solid image-flow ribbon edges.                                                | ✅ shipping | manual      | n/a (manual)                                                                                                              |
| Wire creation (click-then-click) | Canvas tap → tap                                     | Click a port then click an aux source (or vice versa) to create a wire; pending-wire store flips between the two halves until a complete pair lands. | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_pending_wire_completes_aux_then_port                                            |
| Wire creation (type rejection)   | Canvas tap → tap                                     | Mismatched source class (e.g. detector when corrector is required) is rejected; state is unchanged and pending-wire is cleared.                      | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_wire_create_rejects_type_mismatch                                               |
| Wire disconnect button           | Per-slot Disconnect button in inspector              | Clicking Disconnect clears the slot's wire while leaving the aux node intact (it remains available for re-wiring or eventual orphan-drop on save).   | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_wire_delete_orphans_aux_node                                                    |
| Aux port multi (add slot)        | ``+`` button on list-typed aux ports                 | Pressing ``+`` appends an empty slot to a list-typed aux port; the consumer's wire-list grows by one; aux nodes are unaffected.                      | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_port_slot_add_grows_list_only                                                   |
| Aux port multi (remove slot)     | ``×`` button per slot                                | Pressing ``×`` removes that slot (and any wire pointing at it) from a list-typed aux port; downstream slot indices shift.                            | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_port_slot_remove_pops_index                                                     |
| Inspector wired-port row         | Wired slot row in inspector                          | Wired ports render with a grayed-out background and a "CONNECTED" affordance plus a Disconnect button; no inline drill-in is shown for wired params. | ✅ shipping | unit        | tests/unit/gui/test_param_forms.py::TestWiredSlots::test_wired_slot_renders_disconnect_button                             |
| Inspector empty-port row         | Empty slot row in inspector                          | Empty slots render with a dashed border and a "click to wire" prompt; no Disconnect button is emitted.                                               | ✅ shipping | unit        | tests/unit/gui/test_param_forms.py::TestWiredSlots::test_empty_slot_renders_no_disconnect                                 |
| Aux orphan warning on save       | Toast on Save                                        | Saving emits a toast warning when orphan aux nodes (no remaining incoming wires) will be dropped; orphans are dropped from the persisted JSON.       | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_save_emits_orphan_warning_toast                                                 |
| Drill-in aux breadcrumb          | Breadcrumb segment for aux node                      | Drilling into an aux node (e.g. an aux-of-aux pipeline) pushes a breadcrumb segment so the user can edit the nested scope and navigate back.         | ✅ shipping | integration | tests/gui/builder/test_aux_ports.py::test_drill_in_aux_pushes_breadcrumb                                                  |
