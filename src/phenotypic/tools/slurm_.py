...
def _run_slurm_command(cmd: List[str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        result = subprocess.run(cmd + ["--json"], capture_output=True, text=True, check=False)
...
def get_partition_stats(partition: str) -> Dict[str, Any]:
    ...
    if json_data and "partitions" in json_data:
        ...
        stats["MaxMemPerNode"] = parse_slurm_value(part_info.get("memory", {}).get("maximum", "UNLIMITED"))
        stats["MaxJobs"] = parse_slurm_value(part_info.get("jobs", {}).get("maximum", "UNLIMITED"))
        stats["TotalNodes"] = part_info.get("nodes", {}).get("total", 0)
        stats["TotalCPUs"] = part_info.get("cpus", {}).get("total", 0)
    else:
        # Fallback to regex parsing
        try:
            result = subprocess.run(["scontrol", "show", "partition", partition], capture_output=True, text=True, check=False)
            if result.returncode == 0:
                ...
                stats["MaxMemPerNode"] = parse_slurm_value(find_val(r"MaxMemPerNode=([^\s]+)", output))
                stats["MaxJobs"] = parse_slurm_value(find_val(r"MaxJobs=([^\s]+)", output))
...
def get_user_association(user: str, account: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    cmd = ["sacctmgr", "list", "association", f"user={user}", "format=MaxSubmit,MaxJobs,MaxTRES"]
    if account:
    ...
    if json_data and "associations" in json_data:
        ...
        limits["MaxSubmitJobs"] = parse_slurm_value(assoc.get("max", {}).get("jobs", {}).get("submitted", "UNLIMITED"))
        limits["MaxJobs"] = parse_slurm_value(assoc.get("max", {}).get("jobs", {}).get("active", "UNLIMITED"))
        ...
        try:
            cmd_no_json = ["sacctmgr", "-np", "list", "association", f"user={user}", "format=MaxSubmit,MaxJobs,MaxTRES"]
            if account:
                cmd_no_json.append(f"account={account}")
            
            result = subprocess.run(cmd_no_json, capture_output=True, text=True, check=False)
            if result.returncode == 0 and result.stdout:
                ...
                if len(parts) >= 3:
                    ...
                    for tres_item in tres_str.split(","):
                        if "mem=" in tres_item:
                            limits["MaxMem"] = parse_slurm_value(tres_item.split("=")[1])
                        elif "cpu=" in tres_item:
                            limits["MaxCPUs"] = parse_slurm_value(tres_item.split("=")[1])
        except Exception:
            pass
...
def get_headroom(
    ...
    # submitit uses 'cpus_per_task' (default 1) and 'ntasks_per_node' (default 1)
    ...
    if "mem_gb" in submitit_config:
        ...
        # Default if not specified, though usually required for calculation
    ...
    limit_mem = min(part_stats.get("MaxMemPerNode", float("inf")), assoc_limits.get("MaxMem", float("inf")))
    limit_cpus = min(part_stats.get("TotalCPUs", float("inf")), assoc_limits.get("MaxCPUs", float("inf")))
    limit_jobs_active = min(part_stats.get("MaxJobs", float("inf")), assoc_limits.get("MaxJobs", float("inf")))
    ...
    adj_limit_mem = limit_mem * buffer_ratio if limit_mem != float("inf") else float("inf")
    adj_limit_cpus = limit_cpus * buffer_ratio if limit_cpus != float("inf") else float("inf")
    adj_limit_jobs_active = limit_jobs_active * buffer_ratio if limit_jobs_active != float("inf") else float("inf")
    adj_limit_jobs_submit = limit_jobs_submit * buffer_ratio if limit_jobs_submit != float("inf") else float("inf")

    headroom_mem = (adj_limit_mem - curr_mem) / req_mem if req_mem > 0 else float("inf")
    headroom_cpus = (adj_limit_cpus - curr_cpus) / req_cpus if req_cpus > 0 else float("inf")
    headroom_jobs_active = adj_limit_jobs_active - curr_jobs_active
    headroom_jobs_submit = adj_limit_jobs_submit - curr_jobs_total

    # The actual headroom is the minimum of all bottlenecks
    possible_jobs = min(headroom_mem, headroom_cpus, headroom_jobs_active, headroom_jobs_submit)
...
def validate_submission(
    ...
    # Identify specific bottleneck for better error messaging
    # (Simplified check for the user)
    footprint = context["footprint"]
    assoc = context["association"]
    if footprint["RunningJobs"] + footprint["PendingJobs"] >= assoc.get("MaxSubmitJobs", float("inf")):
