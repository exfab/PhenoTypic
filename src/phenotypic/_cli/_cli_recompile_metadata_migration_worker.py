    ...
    *,
    ...
    try:
        ...
        if result.status not in {"applied", "compatible"}:
            ...
            raise RuntimeError(f"metadata migration {result.status}: {details}")
        with generation_publication_guard(output_dir, slurm_generation):
            atomic_write_json(
                status_path,
                {
...
def _validate_manifest_identity(
    ...
    *,
    ...
    if Path(str(manifest.get("output_dir", ""))).resolve() != output_dir:
        raise ValueError("Migration manifest output directory does not match script")
...
def _load_target(
    manifest: dict[str, Any], task_index: int
) -> dict[str, Any]:
...
def _validate_manifest_target(
    ...
    if Path(task_manifest).resolve() != expected_manifest.resolve():
        raise ValueError("Migration manifest is outside its authoritative plan")
    ...
    if candidate.status == "migratable":
        planned_fields = {
            ...
            "proposed_header_map": [list(pair) for pair in candidate.proposed_header_map],
