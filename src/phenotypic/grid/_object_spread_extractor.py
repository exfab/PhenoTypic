...
from phenotypic.util.constants_ import GRID_DEP, OBJECT, BBOX
...
class ObjectSpreadExtractor(GridMeasureFeatures):
    ...
    def _operate(image: GridImage) -> pd.DataFrame:
        ...
        gs_counts = pd.DataFrame(gs_table.loc[:, GRID_DEP.GRID_SECTION_NUM].value_counts())

        obj_spread = []
        for gs_bindex in gs_counts.index:
            curr_gs_subtable = gs_table.loc[gs_table.loc[:, GRID_DEP.GRID_SECTION_NUM] == gs_bindex, :]
