...
from phenotypic.util.constants_ import OBJECT, GRID_DEP, BBOX
...
class OptimalCenterGridFinder(GridFinder):
    ...
    def _find_padding_midpoint_error(self, pad_sz, image, axis, row_pad=0, col_pad=0) -> float:
        ...
        if axis == 0:
            ...
            current_obj_midpoints = (current_grid_info.loc[:, [str(BBOX.CENTER_RR), GRID_DEP.GRID_ROW_NUM]]
                                     .groupby(GRID_DEP.GRID_ROW_NUM, observed=False)[str(BBOX.CENTER_RR)]
                                     .mean().values)
            ...
            current_obj_midpoints = (current_grid_info.loc[:, [str(BBOX.CENTER_CC), GRID_DEP.GRID_COL_NUM]]
                                     .groupby(GRID_DEP.GRID_COL_NUM, observed=False)[str(BBOX.CENTER_CC)]
                                     .mean().values)
    ...
    def _get_grid_info(self, image: Image, row_padding: int = 0, column_padding: int = 0) -> pd.DataFrame:
        ...
        # Add row number info
        info_table.loc[:, GRID_DEP.GRID_ROW_NUM] = pd.cut(
        ...
        info_table.loc[:, GRID_DEP.GRID_ROW_INTERVAL] = pd.cut(
            info_table.loc[:, str(BBOX.CENTER_RR)],
            bins=row_edges,
        ...
        info_table.loc[:, GRID_DEP.GRID_COL_NUM] = pd.cut(
        ...
        info_table.loc[:, GRID_DEP.GRID_COL_INTERVAL] = pd.cut(
            info_table.loc[:, str(BBOX.CENTER_CC)],
            bins=col_edges,
        ...
        # Grid Section Info
        info_table.loc[:, GRID_DEP.GRID_SECTION_IDX] = list(zip(
            info_table.loc[:, GRID_DEP.GRID_ROW_NUM],
            info_table.loc[:, GRID_DEP.GRID_COL_NUM],
        ),
        )

        idx_map = np.reshape(np.arange(self.nrows * self.ncols), newshape=(self.nrows, self.ncols))
        for idx in np.sort(np.unique(info_table.loc[:, GRID_DEP.GRID_SECTION_IDX].values)):
            info_table.loc[info_table.loc[:, GRID_DEP.GRID_SECTION_IDX] == idx, GRID_DEP.GRID_SECTION_NUM] = idx_map[idx[0], idx[1]]

        # Reduce memory consumption with categorical labels
        info_table.loc[:, GRID_DEP.GRID_SECTION_IDX] = info_table.loc[:, GRID_DEP.GRID_SECTION_IDX].astype('category')
        info_table[GRID_DEP.GRID_SECTION_NUM] = info_table[GRID_DEP.GRID_SECTION_NUM].astype(int).astype('category')
