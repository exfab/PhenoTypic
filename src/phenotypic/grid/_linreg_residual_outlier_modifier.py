...
from phenotypic.util.constants_ import GRID_DEP, GRID_LINREG_STATS_EXTRACTOR
...
class GridAlignmentOutlierRemover(GridMapModifier):
    ...
    def _operate(self, image: GridImage) -> GridImage:
        ...
        if self.axis is None or self.axis == 0:
            ...
            row_variance = grid_info.groupby(GRID_DEP.GRID_ROW_NUM)[GRID_LINREG_STATS_EXTRACTOR.RESIDUAL_ERR].std()

            #   Divide standard deviation by mean
            row_variance = row_variance\
                           / grid_info.groupby(GRID_DEP.GRID_ROW_NUM)[GRID_LINREG_STATS_EXTRACTOR.RESIDUAL_ERR].mean()
            ...
            for row_idx in over_limit_row_variance.index:
                row_err = grid_info.loc[
                    grid_info.loc[:, GRID_DEP.GRID_ROW_NUM] == row_idx,
        ...
        if self.axis is None or self.axis == 1:
            # Calculate the coefficient of variance (std/mean)
            #   Collect the standard deviation
            col_variance = grid_info.groupby(GRID_DEP.GRID_COL_NUM)[GRID_LINREG_STATS_EXTRACTOR.RESIDUAL_ERR].std()

            #   Divide standard deviation by mean
            col_variance = col_variance / grid_info.groupby(GRID_DEP.GRID_COL_NUM)[GRID_LINREG_STATS_EXTRACTOR.RESIDUAL_ERR].mean()
            ...
            for col_idx in over_limit_col_variance.index:
                col_err = grid_info.loc[
                    grid_info.loc[:, GRID_DEP.GRID_COL_NUM] == col_idx,
                    GRID_LINREG_STATS_EXTRACTOR.RESIDUAL_ERR
                ]
