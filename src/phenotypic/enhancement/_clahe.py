...
class CLAHE(ImageEnhancer):
    """
    Applies Contrast Limited Adaptive Histogram Equalization (CLAHE) to an _parent_image.

    This class is used to preprocess images by applying the CLAHE algorithm, which
    enhances the contrast of an _parent_image by adjusting the local contrast within specified
    ...
    Parameters:
        kernel_size (int): The size of the kernel used for the local histogram. If not
            provided, an adaptive size based on the _parent_image dimensions is used.
    """
