...
class MedianEnhancer(ImageEnhancer):
    """
    The MedianEnhancer class applies a median filter operation to an _parent_image's enhanced matrix using
    specified boundary conditions and a constant fill other_image if required.

    This preprocessor enhances _parent_image data by replacing each pixel other_image with the median of
    ...
    Attributes:
        mode (str): Defines the behavior at _parent_image boundaries, where valid options are
            'nearest', 'reflect', 'constant', 'mirror', and 'wrap'.
        cval (float): The constant other_image filled in cells beyond the _parent_image boundary when
