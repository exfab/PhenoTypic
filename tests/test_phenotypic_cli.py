...
class TestPhenotypicCLI:
    ...
    def test_cli_missing_pipeline_arg(self, runner):
        ...
        assert result.exit_code != 0
        assert 'PIPELINE_JSON' in result.output or 'Error' in result.output or 'missing' in result.output.lower()
    ...
    def test_cli_missing_output_dir_arg(self, runner, circular_pipeline_json, temp_dirs):
    ...
    def test_cli_invalid_pipeline_file(self, runner, temp_dirs):
        ...
        assert result.exit_code != 0
        assert 'exists' in result.output.lower() or 'Error' in result.output or 'not found' in result.output.lower()
    ...
    def test_cli_no_images_in_input_dir(self, runner, circular_pipeline_json, temp_dirs):
    ...
    def test_cli_creates_output_directories(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
    ...
    def test_cli_invalid_image_type_option(self, runner, circular_pipeline_json, temp_dirs):
        ...
        result = runner.invoke(main, [
            ...
            str(input_dir),
            str(output_dir),
            '--image-type', 'InvalidType'
        ])
        assert result.exit_code != 0
        assert 'Invalid value' in result.output or 'choice' in result.output.lower()

    def test_cli_gridimage_options(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
    ...
    def test_cli_bit_depth_option(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
    ...
    def test_cli_n_jobs_option(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
        """Test n-jobs option."""
        input_dir, output_dir = temp_dirs
        ...
        result = runner.invoke(main, [
            ...
            str(input_dir),
            str(output_dir),
            '--n-jobs', '1'  # Use 1 job for testing to avoid parallelization issues
        ])

        # Should succeed
        assert result.exit_code == 0

    def test_cli_master_csv_created(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
    ...
    def test_cli_output_structure(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
        ...
        for csv_file in csv_files:
            ...
            assert csv_file.stat().st_size > 0, f"CSV file {csv_file.name} should not be empty"
        ...
        for png_file in png_files:
            ...
            assert png_file.suffix == '.png', f"File {png_file.name} should have .png extension"
            assert png_file.stat().st_size > 0, f"PNG file {png_file.name} should not be empty"

    def test_cli_measurements_are_valid_csv(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
        ...
        for csv_file in csv_files:
            ...
            assert len(df_individual) > 0, f"{csv_file.name} should contain measurement data"

    def test_cli_successful_full_workflow(self, runner, circular_pipeline_json, synthetic_grid_image, temp_dirs):
        ...
        result = runner.invoke(main, [
            ...
            str(output_dir),
            '--image-type', 'GridImage',
            '--nrows', '1',  # Synthetic image is 1x1 grid
            '--ncols', '1',
            '--n-jobs', '1'
        ])
    ...
    def test_cli_processes_multiple_synthetic_plates(self, runner, circular_pipeline_json):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(df) > 10, f"Master CSV should have many measurement rows, got {len(df)}"

    @pytest.mark.slow
    def test_cli_synthetic_plates_output_validation(self, runner, circular_pipeline_json):
    ...
    @pytest.mark.slow
    def test_cli_parallel_processing_synthetic_plates(self, runner, circular_pipeline_json):
