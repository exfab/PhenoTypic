...
def test_combined_chain_has_one_afterok_barrier_and_singleton_finalizer(
    ...
    _write_legacy_hdf(output_dir / "results" / "plate_a" / "hdf" / "img1.h5")
    _write_measurement(
    ...
    with (
        patch(
            "phenotypic.phenotypicCLI.get_slurm_array_limit", return_value=8
        ),
        patch(
...
def test_worker_and_finalizer_validate_receipt_and_canonical_bundle(
    ...
    save_processing_state(
        ProcessingState(
            ...
            last_updated=now,
            datasets={"plate_a": DatasetState(initial_images={"img1.tif"})},
            config={
                "success_markers_required": True,
                "work_ids": {"plate_a": {"img1.tif": "work-1"}},
    ...
    publish_image_success(
        ...
        work_id="work-1",
        dataset="plate_a",
        relative_image_path="plate_a/img1.tif",
        image_stem="img1",
        mode="full",
        attempt_id=generation,
        lifecycle_epoch=generation,
        artifacts={"hdf": hdf_path},
    )
    ...
    target_status = json.loads(
        metadata_migration_task_status_path(plan.manifest_path, 0).read_text(
            encoding="utf-8"
        )
    )
    ...
    final_status = json.loads(
        metadata_migration_finalizer_status_path(plan.manifest_path).read_text(
            encoding="utf-8"
        )
    )
...
def test_attempt_scoped_shards_ignore_stale_prior_attempt(
    tmp_path: Path,
) -> None:
    ...
    _write_measurement(
        old_attempt / "measurement_shards" / "shard_0.parquet", 1
    )
    _write_measurement(
        old_attempt / "measurement_shards" / "shard_9.parquet", 99
    )
    _write_measurement(
        new_attempt / "measurement_shards" / "shard_0.parquet", 2
    )
...
def test_stale_ordinary_worker_cannot_write_shard_or_status(
    ...
    source = (
        output_dir / "results" / "plate_a" / "measurements" / "img.parquet"
    )
...
def test_superseded_finalizer_rechecks_before_master_publication(
    ...
    def supersede_while_finalizer_waits(
        *_args: object,
...
def test_stale_migration_finalizer_cannot_publish_or_deactivate_new_owner(
    ...
    _write_legacy_hdf(output_dir / "results" / "plate_a" / "hdf" / "img.h5")
    initialize_slurm_lifecycle(
        output_dir, generation=old_generation, mode="recompile"
    )
    plan = plan_metadata_schema_for_slurm_recompile(
