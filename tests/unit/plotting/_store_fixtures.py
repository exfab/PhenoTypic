...
`TEST_RUN` is the one run folder these fixtures write (spec §1a); `run_path`
spells a store-relative path inside it.
"""
...
TEST_RUN = FigureRun(date="2026-09-22", pipeline_sha256="ab" * 32)


def run_path(relative: str, run: FigureRun = TEST_RUN) -> str:
    """``figures/<run>/<relative>``: where *relative* lives in a run folder."""
    return f"figures/{run.run_id}/{relative}"


def figure_store(root: Path, *stored: StoredFigures) -> Path:
    """Write a promoted-looking store under *root*, one run folder per value."""
...
def emit_image_via_store(coordinator, image=None, *, dataset="ds", image_stem="plate-1"):
    ...
    stored = build_image_figures(
        coordinator._pipeline, object() if image is None else image, run=TEST_RUN
    )
    if stored is None:
        return None
    ...
    try:
        ...
        coordinator.publish_store_figures(
            store, run_id=stored.run.run_id, dataset=dataset, image_stem=image_stem
        )
    finally:
