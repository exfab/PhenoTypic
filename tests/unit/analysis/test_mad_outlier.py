...
class TestMADOutlierRemover:
    """Test suite for MADOutlierRemover functionality."""
    ...
    def test_initialization_invalid_threshold(self):
        ...
        with pytest.raises(ValueError, match="threshold must be positive"):
            MADOutlierRemover(on="Area", groupby=["ImageName"], threshold=-1.0)

        with pytest.raises(ValueError, match="threshold must be positive"):
            MADOutlierRemover(on="Area", groupby=["ImageName"], threshold=0)

    def test_analyze_basic(self, sample_data):
        """Test basic outlier removal functionality."""
        detector = MADOutlierRemover(on="Area", groupby=["ImageName"], threshold=3.5)
        ...
        assert "_is_outlier" not in filtered_data.columns
    ...
    def test_analyze_removes_outliers(self, sample_data):
        ...
        for group_name in sample_data["ImageName"].unique():
            ...
            # Min value in filtered should be >= min in original
            assert filtered_group["Area"].min() >= original_group["Area"].min()

    def test_analyze_different_threshold_values(self, sample_data):
        """Test that different threshold values produce different results.

        Unlike Tukey's ``k``, a *lower* MAD threshold is more aggressive, so it
        removes at least as many rows as a higher threshold.
        """
        detector_strict = MADOutlierRemover(
            on="Area", groupby=["ImageName"], threshold=2.5
        )
        detector_lenient = MADOutlierRemover(
            on="Area", groupby=["ImageName"], threshold=5.0
        )

        filtered_strict = detector_strict.analyze(sample_data)
        filtered_lenient = detector_lenient.analyze(sample_data)

        # Lower threshold should remove at least as many outliers
    ...
    def test_analyze_multiple_groups(self, sample_data_multiple_groups):
        ...
        detector = MADOutlierRemover(
            on="Area", groupby=["Plate", "ImageName"], threshold=3.5
        )
        ...
        # Each group should have <= original rows
        for group_key in original_groups.index:
            assert filtered_groups[group_key] <= original_groups[group_key]

    def test_results_method(self, sample_data):
        """Test the results() method returns filtered data."""
        detector = MADOutlierRemover(on="Area", groupby=["ImageName"])
    ...
    def test_show_method_individual(self, sample_data):
        """Test show() method with collapsed=False."""
        detector = MADOutlierRemover(on="Area", groupby=["ImageName"])

        detector.analyze(sample_data)
        fig, axes = detector.show(collapsed=False)
    ...
    def test_show_with_criteria_single_filter(self, sample_data_multiple_groups):
        ...
        detector = MADOutlierRemover(on="Area", groupby=["Plate", "ImageName"])
    ...
    def test_parallel_processing(self, sample_data_multiple_groups):
        ...
        detector_parallel = MADOutlierRemover(
            on="Area", groupby=["Plate", "ImageName"], n_jobs=2
        )
        detector_sequential = MADOutlierRemover(
    ...
    def test_no_outliers_in_data(self):
        ...
        assert len(filtered_data) >= len(data) * 0.95
    ...
    def test_apply2group_func_static_method(self):
        ...
        filtered = MADOutlierRemover._apply2group_func(
            key=None, group=group, on="Area", threshold=3.5
        )
    ...
    def test_single_group_single_value(self):
        ...
        # One value per group -> MAD is 0, all values kept
        filtered_data = detector.analyze(data)
    ...
    def test_nan_values_in_measurement_column(self):
        ...
        # Result should be a valid DataFrame; NaN rows are preserved
        assert isinstance(filtered_data, pd.DataFrame)
        assert filtered_data["Area"].isna().sum() == 2

    def test_mad_zero_fallback(self):
        """Test the mean-absolute-deviation fallback when MAD is zero.

        A group where > 50% of values are identical has MAD == 0. The test
        must fall back to the mean absolute deviation and still flag the
        deviating rows; a fully-identical group must return unchanged.
        """
        # 12 identical values + 3 clearly deviating ones -> MAD == 0
        data = pd.DataFrame(
            {
                "ImageName": ["img1"] * 15,
                "Area": [100.0] * 12 + [100.0, 500.0, 900.0],
            }
        )

        detector = MADOutlierRemover(on="Area", groupby=["ImageName"], threshold=3.5)
        filtered = detector.analyze(data)

        # The two large deviations should be flagged by the mean-AD fallback
        assert 900.0 not in filtered["Area"].values
        assert len(filtered) < len(data)

        # A fully-identical group has no possible outliers
        identical = pd.DataFrame(
            {"ImageName": ["img1"] * 10, "Area": [42.0] * 10}
        )
