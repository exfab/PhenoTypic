...
def test_edge_correction_is_abstract_setanalyzer():
    ...
    with pytest.raises(TypeError):
        EdgeCorrection(on="Size_Area", groupby=["Metadata_Strain"])
...
def test_edge_correction_validates_grid():
    ...
    with pytest.raises(ValueError):
        _Concrete(on="Size_Area", groupby=["g"], connectivity=5)
    with pytest.raises(ValueError):
        _Concrete(on="Size_Area", groupby=["g"], nrows=0)
    ok = _Concrete(on="Size_Area", groupby=["g"], nrows=8, ncols=12, connectivity=8)
    assert ok.ncols == 12
