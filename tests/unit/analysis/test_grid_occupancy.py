...
def _layout(image_file: str = "plate1.png", n_cells: int = 96) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        "Metadata_ImageFile": [image_file] * n_cells,
...
def _measurements(
    ...
    """
    return pd.DataFrame({
        "Metadata_ImageFile": [image_file] * len(cell_ids),
...
class TestPublicSurface:
    ...
    def test_on_defaults_to_label(self) -> None:
        ...
        chk = GridOccupancy(metadata=_layout(), groupby=["Metadata_ImageFile"])
...
class TestUnmatchedGroup:
    ...
    def test_unmatched_group_fails_and_is_recorded(self) -> None:
        chk = GridOccupancy(
            metadata=_layout("plate1.png"), groupby=["Metadata_ImageFile"]
        )
