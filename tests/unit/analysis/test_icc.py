...
subjects are ``Metadata_Time``, raters are ``Metadata_BioReplicate``.
"""
...
from phenotypic.schema import CULTURE, IMAGE, SAMPLE
...
def _make_group(matrix: np.ndarray, plate: str = "P1") -> pd.DataFrame:
    ...
    columns are raters (``Metadata_BioReplicate``).
    """
    rows = []
    ...
    for s in range(n_subjects):
        for r in range(n_raters):
            rows.append({
                "Plate": plate,
                str(IMAGE.IMAGE_NAME): f"{plate}_t{s}.png",
                "Object_Label": s * n_raters + r + 1,
                "Metadata_Time": s,
                "Metadata_BioReplicate": r + 1,
...
class TestNanGuardPaths:
    ...
    def test_incomplete_matrix_is_nan_not_loud(self) -> None:
        ...
        data = pd.DataFrame({
            ...
            "Metadata_BioReplicate": [1, 2, 1, 2, 1],
...
class TestDisagreementSemantics:
    ...
    def test_anticorrelated_raters_negative_icc_fails(self) -> None:
        ...
        data = pd.DataFrame({
            ...
            "Metadata_BioReplicate": [1, 2, 1, 2, 1, 2],
...
class TestBehavioralEdges:
    ...
    def test_default_axis_labels(self) -> None:
        chk = ICC(on="Size_Area", groupby=["Plate"])
        assert chk.subject_label == str(CULTURE.TIME)
        assert chk.rater_label == str(SAMPLE.BIO_REPLICATE)
