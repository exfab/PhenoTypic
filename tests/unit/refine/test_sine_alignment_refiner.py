"""Tests for RefineBySineFit — NCC-specific and parameter tests.
...
from phenotypic.refine import RefineBySineFit
...
class TestSineAlignmentRefinerCorrelation:
    ...
    def test_ncc_perfect_match(self):
        ...
        ncc = RefineBySineFit._normalized_cross_correlation(signal, template)
...
class TestSineAlignmentRefinerParameters:
    ...
    def test_refiner_with_custom_parameters(self):
        """Test RefineBySineFit with custom parameters."""
        refiner = RefineBySineFit(
                smoothing_sigma=1.5,
    ...
    def test_correlation_threshold_effect(self, synth_plate):
        ...
        refiner_low = RefineBySineFit(correlation_threshold=0.1)
        refined_low = refiner_low.apply(detected)

        refiner_high = RefineBySineFit(correlation_threshold=0.8)
    ...
    def test_smoothing_sigma_effect(self, synth_plate):
        ...
        refiner_no_smooth = RefineBySineFit(smoothing_sigma=0.0)
        refined_no_smooth = refiner_no_smooth.apply(detected)

        refiner_smooth = RefineBySineFit(smoothing_sigma=2.0)
    ...
    def test_edge_refinement_toggle(self, synth_plate):
        ...
        refiner_no_refine = RefineBySineFit(edge_refinement=False)
        refined_no_refine = refiner_no_refine.apply(detected)

        refiner_refine = RefineBySineFit(edge_refinement=True)
    ...
    def test_json_roundtrip(self):
        """Test JSON serialization roundtrip of RefineBySineFit in pipeline."""
        ...
        assert len(ops) == 2
        refiner = ops["RefineBySineFit"]
        assert isinstance(refiner, RefineBySineFit)
        assert refiner.smoothing_sigma == 1.5
