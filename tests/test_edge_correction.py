...
class TestSurroundedPositionsValidation:
    ...
    def test_bad_connectivity_raises(self):
        ...
        with pytest.raises(ValueError, match="connectivity must be 4 or 8"):
            EdgeCorrector.surrounded_positions(active, shape, connectivity=6)
        
        with pytest.raises(ValueError, match="connectivity must be 4 or 8"):
            EdgeCorrector.surrounded_positions(active, shape, connectivity=3)
    ...
    def test_out_of_bounds_index_raises(self):
        ...
        with pytest.raises(ValueError, match=f"All active_idx must be in \\[0, {total}\\)"):
            EdgeCorrector.surrounded_positions([-1], shape)
        
        # Index too large
        with pytest.raises(ValueError, match=f"All active_idx must be in \\[0, {total}\\)"):
            EdgeCorrector.surrounded_positions([9], shape)
        
        # Mixed valid and invalid
        with pytest.raises(ValueError, match=f"All active_idx must be in \\[0, {total}\\)"):
            EdgeCorrector.surrounded_positions([0, 4, 10], shape)
    ...
    def test_min_neighbors_zero_raises(self):
        ...
        with pytest.raises(ValueError, match="min_neighbors must be in \\[1, 4\\]"):
            EdgeCorrector.surrounded_positions(active, shape, connectivity=4, min_neighbors=0)
    ...
    def test_min_neighbors_exceeds_connectivity_raises(self):
        ...
        with pytest.raises(ValueError, match="min_neighbors must be in \\[1, 4\\]"):
            EdgeCorrector.surrounded_positions(active, shape, connectivity=4, min_neighbors=5)
        
        with pytest.raises(ValueError, match="min_neighbors must be in \\[1, 8\\]"):
            EdgeCorrector.surrounded_positions(active, shape, connectivity=8, min_neighbors=9)
    ...
    def test_invalid_shape_raises(self):
        ...
        with pytest.raises(ValueError, match="shape must be two positive integers"):
            EdgeCorrector.surrounded_positions(active, (0, 3))
        
        with pytest.raises(ValueError, match="shape must be two positive integers"):
            EdgeCorrector.surrounded_positions(active, (3, 0))
        
        with pytest.raises(ValueError, match="shape must be two positive integers"):
            EdgeCorrector.surrounded_positions(active, (-1, 3))
...
class TestSurroundedPositionsGeometry:
    ...
    def test_border_never_qualify_connectivity4_none(self):
        ...
        result = EdgeCorrector.surrounded_positions(
    ...
    def test_corner_has_two_neighbors_connectivity4(self):
        ...
        result, counts = EdgeCorrector.surrounded_positions(
...
class TestSurroundedPositionsDegenerate:
    ...
    def test_empty_active_idx_returns_empty(self):
        ...
        result = EdgeCorrector.surrounded_positions([], (5, 5), connectivity=4)
        assert len(result) == 0
    ...
    def test_single_active_cell_returns_empty_when_none(self):
        ...
        result = EdgeCorrector.surrounded_positions([4], (3, 3), connectivity=4)
        assert len(result) == 0
...
class TestSurroundedPositionsCorrectness:
    ...
    def test_3x3_block_connectivity4(self):
        ...
        result = EdgeCorrector.surrounded_positions(active, (rows, cols), connectivity=4)
    ...
    def test_3x3_block_connectivity8(self):
        ...
        result = EdgeCorrector.surrounded_positions(active, (rows, cols), connectivity=8)
        expected = np.array([4 * cols + 6], dtype=np.int64)
        assert np.array_equal(result, expected)
    ...
    def test_subset_property_connectivity4(self):
        ...
        res4 = set(EdgeCorrector.surrounded_positions(
            active, (rows, cols), connectivity=4, min_neighbors=4
        ))
        res3 = set(EdgeCorrector.surrounded_positions(
            active, (rows, cols), connectivity=4, min_neighbors=3
        ))
        res2 = set(EdgeCorrector.surrounded_positions(
            active, (rows, cols), connectivity=4, min_neighbors=2
        ))
        res1 = set(EdgeCorrector.surrounded_positions(
    ...
    def test_subset_property_connectivity8(self):
        ...
        res8 = set(EdgeCorrector.surrounded_positions(
            active, (rows, cols), connectivity=8, min_neighbors=8
        ))
        res7 = set(EdgeCorrector.surrounded_positions(
            active, (rows, cols), connectivity=8, min_neighbors=7
        ))
        res6 = set(EdgeCorrector.surrounded_positions(
...
class TestSurroundedPositionsMisc:
    ...
    def test_horizontal_line(self):
        ...
        result2 = EdgeCorrector.surrounded_positions(
            active, (rows, cols), connectivity=4, min_neighbors=2
        )
    ...
    def test_checkerboard_pattern(self):
        ...
        result8 = EdgeCorrector.surrounded_positions(
