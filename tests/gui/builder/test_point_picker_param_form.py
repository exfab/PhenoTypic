...
def test_picker_widget_replaces_input_for_manual_point_detector():
    ...
    centers_inputs = [
        ...
        and getattr(node, "id", {}).get("type")
        in ("param-list", "param-tuple", "param-str")
    ]
...
def test_picker_store_carries_initial_centers():
    ...
    for node in _walk(form):
        ...
        if (
            isinstance(nid, dict)
            and nid.get("type") == "param-point-picker-store"
        ):
            assert node.data == [[10, 20], [30, 40]] or node.data == [
                [10.0, 20.0],
                [30.0, 40.0],
            ]
            return
