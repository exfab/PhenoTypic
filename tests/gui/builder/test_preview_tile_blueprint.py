"""Preview tile blueprint: stage store layer -> DZI; reject bad components."""
import numpy as np
import pytest
...
def _seed_scope_store(session_id, block_id):
    ...
    img.save2zarr(sdir / "base_00.ome.zarr")
    manifest = {"version": pc.MANIFEST_VERSION,
                "fingerprint": "fp", "scope_key": "", "error": None,
                "nodes": {block_id: {"store": "base_00.ome.zarr",
...
def _seed_scope_store_no_objmap(session_id, block_id):
    """Seed a scope with a store that carries no label image.

    Uses ``save_intermediate_zarr`` with only the non-objmap layers, so the
    ``phenotypic.labels`` key is absent entirely and
    ``Image.load_layer_zarr(store, "objmap")`` raises ``KeyError`` —
    exercising the 404 path for the overlay/objmap channels.
    """
    sdir = pc.scope_dir(session_id, [])
    img = Image(arr=np.zeros((48, 64, 3), dtype=np.uint8))
    img.save_intermediate_zarr(
        sdir / "no_objmap.ome.zarr", layers=("rgb", "gray", "detect_mat"),
    )
    manifest = {"version": pc.MANIFEST_VERSION,
                "fingerprint": "fp", "scope_key": "", "error": None,
                "nodes": {block_id: {"store": "no_objmap.ome.zarr",
...
def test_preview_dzi_served(tmp_path, monkeypatch):
    ...
    sid = "previewsess0001"
    blk = "b" * 32
    shash = _seed_scope_store(sid, blk)  # seed AFTER app creation survives the wipe
...
def test_preview_rejects_bad_channel(tmp_path, monkeypatch):
    ...
    sid = "previewsess0001"
    blk = "b" * 32
    shash = _seed_scope_store(sid, blk)
    client = app.server.test_client()
...
def test_overlay_on_no_objmap_node_returns_404(tmp_path, monkeypatch):
    """Requesting overlay for a node whose store lacks objmap returns 404, not 500."""
    monkeypatch.setattr(pc, "preview_cache_root", lambda: tmp_path / "root")
    app = create_app(image_root=tmp_path)
    sid = "previewsess0002"
    blk = "c" * 32
    shash = _seed_scope_store_no_objmap(sid, blk)
    client = app.server.test_client()
...
def test_stage_channel_png_does_not_publish_partial_final_file(
    ...
    with pytest.raises(RuntimeError, match="simulated write failure"):
        pt.stage_channel_png(tmp_path, "blk", "gray", store_path)
