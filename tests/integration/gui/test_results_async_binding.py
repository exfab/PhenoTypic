...
def test_cancel_racing_final_snapshot_check_cannot_publish(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Cancellation after entering the CAS still preserves the prior pair."""
    output = _seed_output(tmp_path)
    entered_check = threading.Event()
    release_check = threading.Event()
    real_check = results_viewer_module.OutputRoot.require_session_snapshot_current

    def _blocked_check(self: Any, *, context: str) -> None:
        entered_check.set()
        assert release_check.wait(5.0)
        real_check(self, context=context)

    monkeypatch.setattr(
        results_viewer_module.OutputRoot,
        "require_session_snapshot_current",
        _blocked_check,
    )
    shell_app, viewer_session = compose_hub(
        SandboxRoot.from_path(tmp_path),
