...
class ClassMembersDirective(SphinxDirective):
    ...
    def run(self):
        ...
        try:
            ...
            self.state.document.reporter.warning(f"Error importing {class_path}: {e}")
            return []
        ...
        if "properties" in self.options or not self.options:
            for name, member in inspect.getmembers(cls):
                ...
                if isinstance(member, property):
                    ...
                    summary = self.get_docstring_summary(
                        member.fget if member.fget else member
                    )
                    ...
                    attributes_info.append(
                        {
                            ...
                            "obj": member,
                            "summary": str(member)
                            if not hasattr(member, "__doc__") or not member.__doc__
        ...
        if attributes_info:
            ...
            for attr in attributes_info:
                ...
                name_ref = pending_xref(
                    "",
                    refdomain="py",
                    reftype="attr",
                    reftarget=attr["fullname"],
                    refexplicit=True,
                )
