...
| `Refresh` button | Flush the classifier's LRU cache and publish one new source revision. Use this after dropping new files into a directory or moving a CLI output around. Any open Browse state derived from the prior source is retired, rather than silently showing a stale image. |

## Capability badges

Every entry is run through a small classifier
