...
# Usage: bash slurm_behavior_probe.sh <cpu-partition> [<gpu-partition>]
...
echo "== 6. sinfo on an unknown partition (exit status and stdout)"
out=$(sinfo -p does-not-exist --Format=gres --noheader 2>/dev/null); echo "exit=$? stdout=[$out]"
