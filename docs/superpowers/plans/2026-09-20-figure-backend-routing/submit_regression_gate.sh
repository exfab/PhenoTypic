#!/bin/bash
# Submit the figure-backend-routing full regression (plan Task 14) as a 24-way
...
SRC_WORKTREE="${REPO}/.claude/worktrees/figure-backend-routing"
...
WORKTREE="${GATE_ROOT}/fbr-${LABEL}-${SHORT}"
...
ARRAY_ID=$(sbatch --parsable --cpus-per-task=8 --job-name="fbr-${LABEL}" \
