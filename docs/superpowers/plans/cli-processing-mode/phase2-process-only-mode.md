...
3. **SLURM multi-chunk finalize — known limitation (recorded).** The chosen "dependent finalize task" (per user) submits with `--dependency=afterany:<chunk-0 job>`. Under the drip-feed dispatcher only chunk 0's job ID is known at submission time, so a process-only run spanning **multiple array chunks** (more images than `array_limit`) rebuilds the manifest after chunk 0 rather than after all chunks — the `is_complete` signal can appear early (no data loss; all layer files are still written). Single-chunk runs (the common 1-level case) are exact. The strategy now emits a runtime warning for the multi-chunk case and the finalize-script docstring documents it. A fully-correct multi-chunk dependent finalize requires threading submission into the drip-feed completion seam (the alternative, a last-chunk self-elect sentinel, was the design the user chose against) — deferred as a follow-up.
