...
Review-gate fixes applied after Task 9 (both review lenses):

1. **D9 objmap-empty check uses `image.num_objects == 0`** (not the doc's `accessor.isempty()`). `isempty()` returns `shape[0]==0` — true only for a degenerate zero-sized array, never for a real "no detector ran" image. `num_objects == 0` is the correct realization of D9. (Accepted as a refinement; the `getattr` default was dropped since the worker always holds an `Image`/`GridImage`.)
2. **Classifier guard tightened** (`_classifier.py`): `is_process_only_output` now requires `not is_cli_output_results` (i.e. no `results/`). This closes a transient window where a forward run mid-flight (manifest + `results/` written, but `master_measurements.parquet` not yet finalized) was briefly classified as process-only. Process-only runs never write `results/`, so the gate is exact. Test: `test_in_progress_forward_run_not_flagged_process_only`.
3. **SLURM multi-chunk finalize — resolved (Option 2, per user).** The initial "separate dependent finalize task" could only depend on chunk 0's job ID (the only one known at submission time under the drip-feed dispatcher), so a multi-chunk process-only run could rebuild the manifest prematurely. **Fixed** by reusing the forward path's embedded-finalizer mechanism: the **last** process-only chunk now appends a single **manifest-only** sentinel (`_MANIFEST_SENTINEL`) as its final array entry; that task runs `_cli_checkpoint_handler --checkpoint-type manifest` (no aggregation, no dashboard). The drip-feed runs chunks strictly in order, so the manifest is rebuilt after every image across **any** number of chunks — correct, no premature `is_complete`. The standalone `generate_process_only_finalize_script` + its dependent submission were removed (dead). Tests: `test_array_script_threads_process_only_and_omits_aggregation` (manifest sentinel present on the single/last chunk; no aggregation/full-finalizer) and `test_process_only_embeds_manifest_sentinel_on_last_chunk_only` (multi-chunk placement). Chunk sizing already reserves sentinel headroom (`_max_images_per_chunk`), which process-only otherwise leaves unused, so the one extra entry stays within `MaxArraySize`.
