...
| **P2** | [phase-2-identity-schema.md](phase-2-identity-schema.md) | Content-derived `processing_generation`, `restart_epoch` — **including the third minting site in `_cli_migrate.py`** (CAN-7). The `scheduler_epoch` collapse was **withdrawn**, not delivered (§5.1 amendment, user-ruled); Task 4 deleted a dead self-comparison instead. | P1 | stale-worker test against the fence that actually exists (CAN-15) |
