...
- **Legacy-study detector** — ONE module-level helper, `is_legacy_study_present(storage_url: str, study_name: str = _STUDY_NAME) -> bool` in `_strategies/_optuna_support.py` (defined in Phase 2). Returns `True` iff a pre-cutover study (old name `"tune"`, or `direction == maximize`, or missing the `tune_convention` attr) exists in the storage; reads via `optuna.load_study` in `try/except KeyError` (no study → `False`); use `.directions` not `.direction` for the multi-objective case. **Both** Phase 2's `OptunaStudyStore.__init__` guard **and** Phase 4's GUI monitor degrade-message (`gui/tune/_callbacks.py`) call this *same* helper — do not re-implement it. (If Phase 2 inlines it as a private store method, extract it to this shared helper so Phase 4 imports it; the *behavior* — friendly message on a legacy study, never reopening it — is the contract.)
