...
| DA2 | The completed-run publisher lives in `tests/_output_layout.py`, importable without `PLAYWRIGHT=1`. It preserves each fixture's pixel source: it certifies a store the fixture already wrote; otherwise it certifies the fixture's overlay without creating a store (the Results viewer crops from a store in preference to an overlay, so a promoted store would change what the fixture renders); only an image with neither gets a minimal promoted store. The e2e `publish_coherent_terminal_evidence` delegates to it. `_build_sandbox`'s placeholder output (a zero-byte master no viewer can bind) keeps a manifest-only helper. |
...
9. Publication over a fixture's outputs resolves to `completion == "complete"` with mutations enabled, leaves the fixture's `deliverables/` bytes and any existing store unchanged, keeps an overlay-backed image's overlay as its pixel source, and fails loudly on a missing core file or a miscounted fixture — shown by a non-browser test that can fail.
10. With `PLAYWRIGHT=1`, both previously failing `test_analysis_app.py` tests pass. So do the two failures of the same cause hidden from CI by `ci_flaky`: `test_filter_offcanvas.py::test_range_method_filters_picker` (HTTP 423) and `test_radial_triage.py::test_colony_radial_debris_mark_writes_category_parquet` (a disabled badge). No other e2e module that calls the helper gains a failure.
