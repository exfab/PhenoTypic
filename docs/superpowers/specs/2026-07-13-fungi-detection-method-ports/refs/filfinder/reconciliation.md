...
| Existing-mask path skips segmentation | `upstream/fil_finder/filfinder2D.py:299-325` | Always call `create_mask(use_existing_mask=True)` for nonempty masks; suppress only its expected warning. |
...
lengths, and selected label map, rather than checking only the final nonzero count.
