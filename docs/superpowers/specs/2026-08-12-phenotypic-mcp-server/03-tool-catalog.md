...
|---|---|---|
| `insert_op {slot, index, class, params}` | `{kind, slot, class, params}` | an op of `class` **present** in `slot` ⇒ `keep`; absent ⇒ `revert`. **The derivation tests class presence only, not params** — the dominant loop is *insert, probe, keep, then `set_params` to tune*, so a kept insertion no longer carries its original params and matching on them would report `revert` for the commonest success in §8.7. `params` stay in the match key, which asks "is this the same attempt"; the derivation asks "did the insertion survive" |
| `remove_op {slot, index}` | `{kind, slot, class}` — `class` **resolved from `index` at record time**, since the edit itself carries neither `class` nor `params` | **inverted**: the op **absent** from `slot` ⇒ `keep`; present ⇒ `revert`. **`undetermined` when `slot` holds more than one op of that class** — removing one of two `BlurGauss` leaves the class present, so "present ⇒ revert" would report the opposite of the truth |
| `move_op {slot, from, to}` | `{kind, slot, class, order_after}` — `class` resolved from `from` at record time; `order_after` is the slot's ordered class sequence the move produced | `order_after` still equals the slot's current class sequence ⇒ `keep`; otherwise `revert`. **`undetermined` when `slot` holds more than one op of the resolved class** — swapping two ops of the same class leaves the class sequence identical, so the comparison cannot see the move at all | `order_after` still equals the slot's current class sequence ⇒ `keep`; otherwise **`undetermined`** — a later unrelated edit also changes the sequence, and the server cannot attribute the difference |
