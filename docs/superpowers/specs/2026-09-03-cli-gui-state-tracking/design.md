...
> | `lifecycle_epoch` | **Also an on-disk key**, and that is the stronger half: `publish_image_success` writes `"lifecycle_epoch"` into **every image marker** (`_cli_completion.py:234`), so this is the same persistence argument that pins `slurm_generation` — and it survives any future change to the value's meaning, which the mode argument does not. It is *additionally* **mode-dependent at runtime**: `_authoritative_lifecycle_epoch()` returns the scheduler generation under SLURM and the *processing generation* locally, so `scheduler_epoch` would be **narrower than the value**. Persistence forecloses the obvious rebuttal to that second point — *"then narrow the value"* — because narrowing it would still rewrite a key already on disk in every marker of every run. (Added after the P2 gate, SPEC-C1: the row originally gave only the mode argument.) |
