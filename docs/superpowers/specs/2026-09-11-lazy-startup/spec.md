...
| P9 | Design §2 and Appendix A left `_core/_image_parts/plot_accessor/_diagnostics_plotter.py` unchanged for the same reason as P3 | A runtime trace puts it on the `Image`, CLI and hub paths: `plotting/_image_plots.py:52` applies a decorator whose body imports `DiagnosticsPlotter` while the class is being created, which no static import walker sees. Tier 4 would still load plotly and `matplotlib.pyplot`. | It is edited after all: `plt`, `go`, `NAVY` and `OKABE_ITO` move to their point of use. 34 modules are edited in total. Evidence: plan review C1. |
| P10 | Design §3 counts nine heavy CLI import statements | `_cli_interactive` and `_cli_validation` look light but reach the image core: `_cli_validation.py:17` does `from phenotypic import ImagePipeline`. | Eleven statements; `execute_dry_run`, `get_sample_datasets`, `validate_execution_config` and `validate_pipeline` join `_load_cli_runtime()`. Evidence: plan review C2. |
| P11 | Tests, tier 3: the forbidden set is fixed by the post-change measurement | The measurement shows pandas, polars, pyarrow, scipy, skimage, matplotlib and plotly all arriving through shell-side module-level chains (`_gui/analysis/_callbacks.py:25-26`, `_gui/_operation_registry.py:18`, `_gui/run_console/_request_safety.py:15`, and dash's own plotly import). | The asserted set is this spec's minimum — colour, numba, h5py, mahotas, cv2, bm3d and `matplotlib.pyplot`. The rest are listed in the test with their chains, which is what tier 3 already allows. Evidence: plan review C3. |
