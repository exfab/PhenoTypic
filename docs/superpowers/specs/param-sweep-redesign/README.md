...
| [`screening-importance.md`](screening-importance.md) | The `ScreeningPhase`: analysis-first + opt-in 2-round freeze (explore→fANOVA→freeze→focused); hierarchical per-activation importance over the conditional space; cumulative-tail freezing over *total* importance at top-k central tendency; RF+permutation fallback (no Optuna); explore-round unpruned → survivorship-bias-free; multi-objective + wrong-freeze recovery; `param_importance.json`. | master §4 (`ScreeningPhase`), D8 | **✅ written** |
| [`robust-evaluation.md`](robust-evaluation.md) | The `Evaluator`: uniform 3-step loop (`score_image` → robust-aggregate per term → `finalize`), per-term cost `median + λ·IQR` stability penalty (λ=0.5), Scorer-owned normalization (Böck-safe), group-aware CV (replicate-safe; `--cv-group` auto-infer), multi-fidelity pruning + incremental caching, adaptive held-out guard, failure/seeding/degradation policy. Agent-reviewed; carries reciprocal edits to qc §3 + master §4/D4. | master §4 (`Evaluator`), D4 | **✅ written** |
| [`optuna-integration.md`](optuna-integration.md) | `OptunaStrategy`: ask-and-tell control model + conditional materialization; TPE default / CMA-ES native-fallback / NSGA-II auto multi-objective; ASHA pruning via a generic Optuna-free channel; distributed ask-and-tell over a shared journal file or supported external RDB such as PostgreSQL (distributed SQLite is rejected); budget/failure/dead-worker/reproducibility policy; lazy import behind the `tune` extra. | master §4 (`SearchStrategy`), D3, §6 | **✅ written** |
| `mcp-server-design.md` | The `tune_*` tool surface, autonomous vs. steering modes, shared-study session semantics | master §6 | planned |
| [`dash-copilot-design.md`](dash-copilot-design.md) | The `/tune/` Dash co-pilot: attach + delegate-launch to `run_console`; human curation/winner-selection write-back (`user_attrs`, last-write-wins + attribution); on-demand overlays (reuse builder renderer); top-N/Pareto+flagged shortlist; objective-curve / Pareto / importance visuals; optional space-edit (reuse `_param_forms`) → `tuning_spec.json`. Sub-phased 6a→6b→6c; FEATURES/WORKFLOWS/screenshot gates. | master §6, D5/D6 | **✅ written** |
| [`engine-architecture.md`](engine-architecture.md) | **Implementation architecture / ABC layer** bridging the specs to the plans: three tiers (pydantic ABCs `Scorer`/`Evaluator`; Protocols + `StrategyConfig` for `SearchStrategy`/`PruningChannel`; pydantic value-models for domains/proposal/`TuningSpec`); `PolymorphicField` generalizing `OperationField`; the shared `Executor` seam (sweep+tune); the two extension seams; package layout + naming + phasing map. | master §4, §9, §11 | **✅ written** |
| [`e2e-workflows.md`](e2e-workflows.md) | **End-to-end user workflows** — the CLI backend (`python -m phenotypic.tune`: infer → spec → launch → monitor → adopt `best_pipeline.json`) and the Dash co-pilot frontend (6c space-edit → launch-via-`run_console` → 6a monitor → 6b curate winner), unified by one engine + one shared study backend; the connecting artifacts (`tuning_spec.json`/resolved study storage/`best_pipeline.json`), the four human decision points, personas, and the deferred MCP driver. | master §6, D5/D6 | **✅ written** |
