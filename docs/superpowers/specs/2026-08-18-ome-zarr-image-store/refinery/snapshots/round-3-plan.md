...
def assert_store_conforms(store_path: Path) -> None:
    ...
    assert ome_group.is_dir(), f"OME/ group is mandatory under layout 3: {store}"
...
def _assert_reader_level_musts(store: Path, block: dict, ome_xml: str) -> None:
    ...
    from phenotypic.sdk_.ngff_ import LABELS_GROUP, PhenotypicAttr, primary_series

    series_names = list(block[PhenotypicAttr.SERIES].values())

    # 2.2.3 MUST -- "The order of the paths MUST match the order of the Image
    # elements in OME/METADATA.ome.xml." Holds by construction (both derive from
    # series_names), and this is the assertion that makes layout 3 mean anything.
    declared = _group_ome(store)["series"]
    xml_order = re.findall(r'<Image\b[^>]*\bName="([^"]+)"', ome_xml)
    ...
    for name in series_names:
        ...
        for dataset in multiscale["datasets"]:
            ...
            assert array.shape, (name, dataset["path"])
            ...
            assert list(meta["dimension_names"]) == expected_axes, (
                name,
                dataset["path"],
            )
    ...
    assert listed == ["objmap"], listed
    label_group = store / primary / LABELS_GROUP / listed[0]
    label_array = zarr.open_array(store=str(label_group / "0"), mode="r")
    assert np.issubdtype(label_array.dtype, np.integer), (
        "2.6: label pixels MUST be an integer dtype"
    )
    # 2.6 MUST -- the label's datasets array has the same number of levels as
    # the image it labels.
    assert len(_group_ome(label_group)["multiscales"][0]["datasets"]) == (
        level_counts[primary]
    ), "2.6: label level count MUST match the unlabeled image"

    # 1.4 -- the chunk-key separator MUST be uniform store-wide. Asserted through
    # its observable consequence: with "." a chunk key is one path segment, so no
    # level directory holds subdirectories. `.get` chains rather than [] so a
    # writer emitting {"name": "default"} with no configuration fails as an
...
> **The token's payload is written but never read in production (ledger FLOW-20).**
> `read_stage2_token`, `objmap_shape`, and `work_id` have no production consumer anywhere in
> this plan — only Task 3.2's unit tests. And `work_id` is structurally always `None`:
> `stage2_detect_core` has no `work_id` parameter (`_cli_staged_workers.py:138-144`), which is
> why it is hard-coded above. **Keep `objmap_shape`, drop `work_id` from the payload.**
> `objmap_shape` earns its place as a cheap crash-consistency check a future Stage 3 can make
> against the `.npy` it loads; a field that can only ever hold `None` is a field that will be
> misread as meaningful. Update Task 3.2's writer, its schema, and its tests to match.
...
The mutation is **two lines, not one** — seed the store with the raw array first, then read
the store:

```python
    # in stage3_merge_measure_core, replacing `result = load_stage2_raw(...)`:
    image.objmap[:] = load_stage2_raw(output_dir, dataset_name, image_stem)
    result = image.objmap[:]
```
...
first pass kept. Restore the single `load_stage2_raw` call and confirm PASS.

> **The one-line form does not work, and an earlier draft prescribed it (ledger FLOW-15).**
> That draft said to substitute `result = image.objmap[:]` alone and watch the test fail.
> Trace it under the round-2 design: Stage 2 no longer writes the store, so pass 1 loads
> Stage 1's **zeros** → `_write_object_output(image, zeros)` → `drop_frame_background`
> early-returns at `_objmap_accessor.py:503` → the published objmap is zeros and the
> measurements are empty. `simulate_timeout_after_promote()` → pass 2 loads the same zeros →
> identical array, identical (empty) label set. **The mutated code passes.**
>
> The draft then told the executor that a pass means "the fixture is wrong — almost certainly
> no real colony touches the frame." It is not wrong, and they would have chased it. Removing
> the in-store write is what changed this: the store-as-input defect now fails *loudly*
> rather than silently, so reproducing D1 requires putting the raw labels into the store
> first. Seed, then read — that is the state the original defect actually described.
...
- **Every site that deleted the sidecar must now delete BOTH the token and the raw array.**
  ...
  > **Miscount corrected (ledger FLOW-18).** An earlier draft said "five" and Task 3.4's
  > blockquote handled the sixth site separately, describing both deletions there as "plain
  > `.json` unlinks" — i.e. **not** clearing the raw `.npy`. Traced: benign in the normal
  > case, since the token is the gate and Stage 2 overwrites the raw atomically before any
  > reader. But if Stage 1 then fails, that `.npy` is orphaned permanently. Clear both at
  > this site too, and let the count be right — this is the same miscount class as PRE-F5 and
  > B11.
...
- `refresh_success_markers_after_metadata_migration` (`:136-155`) exists because rewriting a
  per-image HDF invalidates the marker's `sha256`. **`--mode migrate` pass 2 does the same
  thing to the per-image measurement parquets** (`results/<ds>/measurements/*.parquet` are
  bundle targets, `_metadata_migration.py:797-845`), which the markers also fingerprint — so
  this bridge must handle store descriptors too. Recorded as OPEN-QUESTIONS **D10**.

  > **The stated reason was a dead one (ledger FLOW-24).** An earlier draft justified this by
  > *"Header-only **store** migration (Phase 5 Task 5.5) rewrites `zarr.json` and does exactly
  > the same thing"* — but Task 5.5 was **cut**. The bridge does still need store handling;
  > the live reason is Task 5.6's republication and the parquet rewrite above.
...
def test_nothing_writes_into_a_promoted_store() -> None:
    ...
    No fixture: this is a module-surface assertion, not a viewer behaviour.
...
already carry tables in this form). All live in `tests/conftest.py` for this phase unless
noted:
...
| `published_store` | dataclass | A promoted store plus its completion marker; `.marker`, `.store`. Shared with Phase 3 Task 3.8. |
...
**Constraints specific to this task:**
- **`migrate` reuses `recompile`'s argument validation exactly.** It joins
  `{full, measure, recompile, process}` in the existing `click.Choice`, rejects `--pipeline`
  **and** `--input`, and names the tree to convert with `--output`. Extend the existing
  `cli_mode in (...)` guards rather than adding a parallel branch — the guards at lines 1231,
  1236, and 1240 each name modes explicitly, so `migrate` is added to the same tuples
  `recompile` already appears in. No guard needs a new exemption.
...
- **`--delete-sources` is the only irreversible step in this plan, and its precondition must
  ...
  Gate each unlink on a positive re-read comparison against its source — layer names, shapes
  and dtypes, the metadata key set, and `phenotypic_work_id` — **plus** a passing
...
- **`--mode migrate` is explicitly TWO-PASS** (user ruling; ledger **MIG-7b**). State both
  ...
  > earlier draft named and which is not a symbol in this codebase.** The
  > preflight → migrate sequence currently lives only inside
  > `migrate_metadata_schema_for_recompile` (`_cli_recompile_metadata_migration.py:44-83`),
  > whose caller Task 5.4 neutralizes; lift the preflight with it.

  Two constraints on pass 1 that follow from running it first:

  - **Skip an `.h5` target whose stem already has a valid store** (ledger **MIG-21**). With
    `keep_source=True` the retained `.h5` files are still enumerated by
    `_discover_bundle_targets` (`:797-810`), so without the skip pass 1 performs the full
    per-HDF header rewrite plus receipts on files nothing will ever read again — immediately
    after Task 5.4 dropped the SLURM fan-out on the grounds that this work is cheap. On a
    first migration nothing is skipped and the cost claim holds; on a re-run everything is.
  - **`--dry-run` suppresses both passes** and reports what each would touch. **An
    interruption between passes is safe to re-run**: pass 1 is idempotent by content (a
    canonical file re-canonicalizes to itself) and pass 2 skips images whose store is already
    valid. A pass-1 failure aborts before any store is written.

  `MigrationReport` gains `headers_migrated: int` and `header_failures: tuple[tuple[Path, str], ...]`
  so a pass-1 failure has somewhere to appear — today it has `converted`/`skipped`/`failed` only.
...
- **Republication is keyed on MARKER state, not on conversion state** (ledger **FLOW-22**).
  ...
  Republish for every image whose store is valid and whose marker does not yet describe it;
  the operation is idempotent, so running it over skipped images costs a marker read.
...
- [ ] Running migrate twice on the same tree converts zero on the second pass **and migrates
      zero headers** — the MIG-21 skip is what makes the second half true.
- [ ] `grep -rn "sbatch" src/phenotypic/_cli/_cli_migrate.py src/phenotypic/sdk_/_hdf_to_zarr.py` returns nothing.
- [ ] **The six** golden fixtures (`v1_flat`, `v2_grouped`, `v2_enh_gray`, `v2_rich`, plus the
      two negative cases) are committed with their generator, and the generator-fidelity check
      of Task 5.1 Step 1a passes.
...
- **Retain the `"hdf"` arm.** `--mode migrate` pass 1 is what reads legacy trees, and it
  reaches the module through exactly these branches. Keep `csv`/`parquet`/`json`/`frame`
  unconditionally — pass 2 needs them.
...
> `dataset_root/"hdf"` and appends every one of them. The set is not empty; without a guard,
> pass 2 rewrites headers into retained files nothing will ever read again, plus receipts.
>
...
**One chrome change is expected: Task 5.7's new `OutputConsistencyReport` reason** — the
banner telling a user their output holds unconverted `.h5` results. It is a new reason on an
existing surface, not a new control or a new view, so per the `gui-tutorial-capture` skill it
needs **no** `FEATURES.md` / `WORKFLOWS.md` row and no tutorial screenshot; run the gates to
confirm they agree. Nothing else in this work changes user-visible chrome — Phase 4 makes
tile rendering faster, not different. If a gate fails on anything other than that banner, a
chrome change slipped in — most likely in Phase 4 or Phase 5 — and needs the skill's full
procedure.

> **Corrected (ledger GEN-31).** An earlier draft asserted the chrome was unchanged outright
> and pointed a failing gate at Phase 4. Round 2's Task 5.7 deliberately adds a user-visible
> banner reason, in Phase **5**, so both halves of that instruction were wrong.
