...
| M7 | `noise_method` outside `{-1, -2} ∪ [0, ∞)` raises `ValueError` | no reference validates it | CAPABILITY | `-1.5` matched neither branch, left `tau = 0.0`, and silently returned `T = ε`. Kovesi's MATLAB errors on the undefined `tau`. The dispatch itself now compares against `epsilon`, as `phasecongruency.jl:512` and `phasecongmono.m:224` do, rather than an invented `1e-9`. |
