...
| M6 | `rayleigh_mode` anchors histogram bins at `0` and retains zeros | our own shipped `_focus_edge_phase.py:512`, which does neither | **CORRECTED to the reference** | Kovesi (`phasecongmono.m:465`): `edges = 0:mx/nbins:mx; n = histc(data, edges)`. `phasepack` (`tools.py:86`) also retains zeros. Our port dropped zeros and let `np.histogram` place edges at `data.min()` — an **undeclared** deviation from all three. **Measured, 2026-07-09** (an earlier revision of this row claimed `0.0130%` on `T`; that figure matches nothing measurable and is withdrawn): on `load_synth_yeast_plate`, `_phasecong3` calls `rayleigh_mode` **once per orientation** (6×), and the returned mode shifts by `8.64e-06` to `3.01e-05` *relative*. The reported `T` is the last orientation's, whose mode shifts by `8.6403e-06` — and `T`'s relative shift is `8.6403e-06` exactly, since `T ∝ tau`. In absolute terms `|ΔT| = 3.39e-08`, and `max|Δpc_sum| = 5.578e-06` over `414 182 / 480 000` pixels. Note the amplitude arrays contain **zero** exact zeros, so the divergence here is the *edge placement*, not the zero-dropping. Neither form is measurably more accurate on synthetic Rayleigh samples (relerr `0.0922` vs `0.0938` at `σ=1`); the reason is faithfulness. **This changes shipped `FocusEdgePhase` output at `noise_method = -2`.** Golden fixture cannot cover it (its `_params` record `noiseMethod = -1`), so the value is pinned by a dedicated test — the mutation audit showed a doubled `rayleigh_mode` was otherwise invisible to every test. |
