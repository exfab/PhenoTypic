...
> This folder holds the design spec as a *set* of documents (placement,
> search-space UX, deploy/monitor, data model). This log tracks the decisions
> and closed open questions behind the interactive mockup
> (`mockups/tune-config-builder.html`).
...
- **#5 Cancel / stop a run** — ✅ *prototyped for Local only* (✕ on local run
  pills → SIGTERM confirm dialog; trials kept + resumable; live count
  decrements). SLURM `scancel` is deferred out of v1.
