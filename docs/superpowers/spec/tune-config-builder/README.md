...
resolutions folded in as D13/D14). One planning-time spike remains before/early
in implementation: confirm `_param_forms.param_form` can render
non-`ImageOperation` scorer pydantic models (else add a scorer registry).
...
switcher, a Local-vs-SLURM live view, local-run cancellation, and an **Export
best pipeline** action that closes the loop (best params → a runnable pipeline
config).
...
| [03-run-deploy-and-monitor.md](03-run-deploy-and-monitor.md) | The Run surface (strategy/budget/compute/deploy), the shared runner extraction, SLURM divergence, local cancellation, and the Monitor export-best loop. |
...
Most of this is GUI work, but three engine/model changes are prerequisites and are
...
3. **`best_params.json`** — add an explicit best-params sidecar so Monitor can
   render the winning params without reverse-engineering `trials.parquet`.
...
- SLURM cancellation from the GUI. v1 monitors SLURM runs through the study store
  when reachable and degrades to a submitted-to-cluster state when not reachable.
