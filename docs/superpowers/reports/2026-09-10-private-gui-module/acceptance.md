...
| 4 | No tracked file outside `docs/superpowers/` names the old package, except the guard test (amended wording) | PASS | Only `tests/unit/gui/test_private_package.py:4,21,22`; the same pattern matches 482 files on `main`. Final review: the tracked binary `.testmondata` also matches (a stale testmon SQLite cache, unchanged from `main`; no workflow reads it); untracking it is a follow-up |
