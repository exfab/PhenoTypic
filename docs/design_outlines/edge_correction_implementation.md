...
Implemented the `_surrounded_positions` function in the `EdgeCorrector` class for detecting colonies that are fully
surrounded by neighbors in a grid-based colony detection system.
...
surrounded = EdgeCorrector._surrounded_positions(active, (rows, cols), connectivity=4)
